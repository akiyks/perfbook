...
\usepackage{ifthen}
\newboolean{newtxtrueslanted}
\newboolean{newtxfontspec}
\input{newtxversion} % load newtx version info (by autodate.sh)
\usepackage[table,svgnames]{xcolor} % newtxtext v1.73 loads xcolor without options
\ifthenelse{\boolean{newtxtrueslanted}}{  % use nice, standard fonts for roman
  \usepackage[defaultsups,helvratio=0.9,trueslanted]{newtxtext}
}{
...
\makeatletter
\renewcommand\p@sublisting{\thelisting}
\makeatother
\captionsetup{labelfont=bf}
\captionsetup[listing]{font=small,labelsep=colon}
% Define "boxedtop" float style
\makeatletter
\let\floatc@boxedtop=\floatc@boxed
\newcommand\fs@boxedtop{\fs@boxed
  \def\@fs@mid{\vspace\belowcaptionskip\relax}%
  \let\@fs@iftopcapt\iftrue}
\makeatother
\floatstyle{plain}
\newcommand\belowsubcapskip{0pt}
\AtEndEnvironment{subcaptionblock}{\vspace*{\belowsubcapskip}}
...
\newboolean{subcaptiongood}
\newboolean{footmiscgood}
\input{subcaption_version}
\newcommand{\IfSubcaptionGood}[2]{\ifthenelse{\boolean{subcaptiongood}}{#1}{#2}}
\newcommand{\IfFootmiscGood}[2]{\ifthenelse{\boolean{footmiscgood}}{#1}{#2}}
\usepackage{tabularx}
...
\newboolean{parskip}
\setboolean{parskip}{true}
\ifdefined\DeclareRelease \else \setboolean{parskip}{false} \fi % need LaTeX 2018-04-01
\newcommand{\IfParSkip}[2]{\ifthenelse{\boolean{parskip}\AND\boolean{ebooksize}}{#1}{#2}}
\IfParSkip{
...
%\RequirePackage{datatool}[=2022-10-01] % not yet needed as of March 2025
...
\else
  ...
  \usepackage{mfirstuc}
  \usepackage[toc,nopostdot,acronym]{glossaries}
...
\newboolean{figcptop}
\setboolean{figcptop}{false}
\newcommand{\IfFigCpTop}[2]{\ifthenelse{\boolean{figcptop}}{#1}{#2}}
...
\newcommand{\IXplx}[3]{\ucindex{#1#3}\hlindex{#1#2}} % put as is into general index for plural of exeptional form
...
\newcommand{\IXhplx}[4]{\indexh{#1 #2#4}{#2#4}{#1}\hlindex{#1 #2#3}}
...
\newcommand{\IXBplx}[3]{\ucindex{#1#3|BF}\hlindex{#1#2}}
...
\newcommand{\IXBhplx}[4]{\indexh{#1 #2#4|BF}{#2#4|BF}{#1}\hlindex{#1 #2#3}}
...
\newcommand{\apiDk}[1]{\hlindex{\co{#1}}\sindex[api]{#1@\co{#1}\categapi{k}|DF}}
\newcommand{\apitk}[1]{\hlindex{\tco{#1}}\sindex[api]{#1@\co{#1}\categapi{k}}}
\newcommand{\DF}[1]{\underline{#1}}
\newcommand{\apiDtk}[1]{\hlindex{\tco{#1}}\sindex[api]{#1@\co{#1}\categapi{k}|DF}}
\newcommand{\apikh}[1]{\hlindex{\co{#1}}\sindex[api]{#1@\co{#1}\categapi{kh}}}
\newcommand{\apipf}[1]{\hlindex{\co{#1}}\sindex[api]{#1@\co{#1}\categapi{pf}}}
\newcommand{\apiDpf}[1]{\hlindex{\co{#1}}\sindex[api]{#1@\co{#1}\categapi{pf}|DF}}
\newcommand{\apitpf}[1]{\hlindex{\tco{#1}}\sindex[api]{#1@\co{#1}\categapi{pf}}}
\newcommand{\apiDtpf}[1]{\hlindex{\tco{#1}}\sindex[api]{#1@\co{#1}\categapi{pf}|DF}}
...
\newcommand{\pplEP}[2]{\hlindex{#1 #2}\sindex[ppl]{#2, #1|EP}}
\newcommand{\pplEPmdl}[2]{\hlindex{#1~#2}\sindex[ppl]{#2, #1|EP}}
\newcommand{\pplEPalt}[2]{\hlindex{#1}\sindex[ppl]{#2|EP}}
\newcommand{\pplEPraw}[1]{\hlindex{#1}\sindex[ppl]{#1|EP}}
...
\ifPDFTeX
  ...
  %msfontstub
\else
  \ifthenelse{\boolean{newtxfontspec}}{
    \IfSansSerif{
      \setmonofont{Inconsolatazi4-Regular.otf}[
        BoldFont=Inconsolatazi4-Bold.otf,
        StylisticSet=2]
    }{}
    \setboolean{nimbusavail} {false} % extra space to escape sed in Makefile
  }{
    \IfSansSerif{
      %msfontstub
      \setboolean{nimbusavail} {true}
    }
  }
\fi
\IfEbookSize{
...
\ifthenelse{\boolean{newtxtrueslanted}}{}{
% Poor person's slanted shape for roman --- newtxtext < v1.65 lacks trueslanted
  \AtBeginDocument{%
...
\else
  \ifthenelse{\boolean{newtxfontspec}}{
    \usepackage{biolinum}
    % restore "TeX Gyre Heros" as sans-serif
    \setsansfont{TeX Gyre Heros}[Scale=0.9,WordSpace=1.05]
  }{
...
\newlength{\myxleftmarginL}
\newlength{\myxleftmarginLL}

\IfTwoColumn{
  \setlength{\myxleftmarginL}{4pt}
  \setlength{\myxleftmarginLL}{3pt}
}{
  \setlength{\myxleftmarginL}{9pt}
  \setlength{\myxleftmarginLL}{9pt}
}
\DefineVerbatimEnvironment{VerbatimL}{Verbatim}%
{numbers=left,numbersep=5pt,xleftmargin=\myxleftmarginL}
\IfParSkip{}{
\AfterEndEnvironment{VerbatimL}{\vspace*{-9pt}}
}
\DefineVerbatimEnvironment{VerbatimLL}{Verbatim}% for snippet inside list
{numbers=left,numbersep=5pt,xleftmargin=\myxleftmarginLL}
\IfParSkip{}{
...
\IfParSkip{
  \newcommand{\vspaceafterverbatiminput}{}
}{
  \newcommand{\vspaceafterverbatiminput}{\vspace*{-9pt}}
}
...
\floatstyle{boxedtop}
\restylefloat{figure}
\renewcommand*\belowsubcapskip{-9pt}
}{}
...
\setindexprenote{%
  ...
  \hfill\begin{minipage}{1.1in}
    \footnotesize%
    \DF{Underline}: Definition.
    \vspace*{1ex}
  \end{minipage}
}
\printindex[api]
