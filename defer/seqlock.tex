...
%
\epigraph{It'll be just like starting over.}{\pplEP{John}{Lennon}}
...
\IXBh{Sequence}{lock}s are used in the Linux kernel for read-mostly data that
...
Readers therefore use the \apik{read_seqbegin()} and \apik{read_seqretry()}
...
Writers therefore use the \apik{write_seqlock()} and \apik{write_sequnlock()}
...
The \apik{seqlock_t} data structure is shown on
\clnrefrange{b}{e}, and contains
the sequence number along with a lock to serialize writers.
\end{fcvref}
\begin{fcvref}[ln:defer:seqlock:impl:init]
\Clnrefrange{b}{e} show \apik{seqlock_init()}, which, as the name indicates,
initializes a \co{seqlock_t}.
\end{fcvref}

\begin{fcvref}[ln:defer:seqlock:impl:read_seqbegin]
\Clnrefrange{b}{e} show \apik{read_seqbegin()}, which begins a sequence-lock
...
will pass to a later call to \apik{read_seqretry()}.
\end{fcvref}
...
\Clnrefrange{b}{e} show \apik{read_seqretry()}, which returns \co{true} if there
...
\Clnrefrange{b}{e} show \apik{write_seqlock()}, which simply acquires the lock,
...
\Clnrefrange{b}{e} show \apik{write_sequnlock()}, which executes a memory barrier
