...
\footnotesize
\newcommand{\bco}[1]{\parbox[t]{1.95in}{\renewcommand{\baselinestretch}{0.8}\selectfont\co{#1}\vspace{3pt}}}
\newlength{\cw}
\begin{tabular}{>{\raggedright\arraybackslash}p{1.8in}
...
\bco{list_for_each_entry_rcu()}
\bco{list_for_each_entry_lockless()} &
    \bco{hlist_for_each_entry_rcu()}
    \bco{hlist_for_each_entry_rcu_bh()}
    \bco{hlist_for_each_entry_rcu_notrace()} &
	\bco{hlist_nulls_for_each_entry_rcu()}
	\bco{hlist_nulls_for_each_entry_safe()} &
	    \bco{hlist_bl_for_each_entry_rcu()} \\
\multicolumn{4}{l}{{\bf Resume traversal}} \\
\bco{list_for_each_entry_continue_rcu()}
\bco{list_for_each_entry_from_rcu()} &
    \bco{hlist_for_each_entry_continue_rcu()}
    \bco{hlist_for_each_entry_continue_rcu_bh()}
    \bco{hlist_for_each_entry_from_rcu()} &
	&
	    \\
