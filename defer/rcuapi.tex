...
\ebresizeverb{0.68}{
\begin{tabularx}{8.63in}{>{\raggedright\arraybackslash}p{0.94in}
    >{\raggedright\arraybackslash}p{1.4in}
    >{\raggedright\arraybackslash}p{1.4in}
...
\apiDk{rcu_read_lock()} and end with \apiDk{rcu_read_unlock()}.
These critical sections can also use  APIs in
\cref{tab:defer:RCU Markers for Read-Side Critical Sections}, including
\apiDk{rcu_read_lock_bh()}, \apiDk{rcu_read_lock_sched()},
and \apiDk{rcu_read_unlock_bh()}, or \apiDk{rcu_read_unlock_sched()}.
...
\apiDk{synchronize_rcu()} and \apiDk{synchronize_rcu_expedited()}, along with
their synonym \apiDk{synchronize_net()}, wait for any type of currently
...
The asynchronous update-side primitive, \apiDk{call_rcu()},
...
The \apiDk{rcu_barrier()} primitive does this job.
...
\QuickQuizB{
	...
	\apik{synchronize_rcu()} invocation, because the
...
\apiDk{SLAB_TYPESAFE_BY_RCU} to \apiDk{kmem_cache_create()}.
...
delimited by \apiDk{srcu_read_lock()} and \apiDk{srcu_read_unlock()}.
...
\apiDk{srcu_struct} for each distinct SRCU usage, so that there is no
...
variable with \apiDk{DEFINE_SRCU()} if the structure must be used in
multiple translation units, or with \apiDk{DEFINE_STATIC_SRCU()} otherwise.
...
\apiDk{synchronize_srcu()} and \apiDk{synchronize_srcu_expedited()}
invocations.
...
asynchronous \apiDk{call_srcu()} function.
...
Also similar to RCU, there is an \apiDk{srcu_barrier()} function that waits
...
context switches.
Updates can use \apiDk{synchronize_rcu_tasks()} to wait for all pre-existing
trampoline execution to complete, or they can use its asynchronous
counterpart, \apiDk{call_rcu_tasks()}.
There is also an \apiDk{rcu_barrier_tasks()} that waits for completion
...
The \apiDk{rcu_assign_pointer()} primitive ensures that any
prior initialization remains ordered before the assignment to the
pointer on weakly ordered machines.
The \apiDk{rcu_replace_pointer()} primitive updates the pointer just like
...
The \apiDk{rcu_pointer_handoff()} primitive simply returns its sole argument,
...
The \apiDk{RCU_INIT_POINTER()} macro can be used to initialize RCU-protected
...
Similarly, \apiDk{RCU_POINTER_INITIALIZER()} provides a \GCC-style
...
In this case, \apiDk{rcu_access_pointer()} may be used.
Normally, however, RCU read-side protection is required, and so the
\apiDk{rcu_dereference()} primitive uses the Linux kernel's \co{lockdep}
...
The \apiDk{rcu_dereference_protected()} API member is provided for this
situation.
...
\apiDk{rcu_dereference_check()}, which also takes a lockdep expression, but
...
In these (hopefully rare) cases, \apiDk{rcu_dereference_raw()} provides
protection but does not check for being invoked within a reader or with
any particular lock being held.
The \apiDk{rcu_dereference_raw_notrace()} API member acts similarly, but
...
that reader with it.
The reader can use the \apiDk{is_a_nulls()} function (which returns \co{true}
if passed an \co{hlist_nulls} \co{NULL} pointer) to determine when
it reaches the end of a list, and the \apiDk{get_nulls_value()} function
...
\apiDk{list_replace_rcu()}, given that this API member gave RCU its name.
...
Debug-object support is automatic for any \apiDk{rcu_head} structures
...
memory allocators can use \apiDk{init_rcu_head()} and \apiDk{destroy_rcu_head()}
at allocation and free time, respectively.
Those using \co{rcu_head} structures allocated on the function-call
stack (it happens!\@) may use \apiDk{init_rcu_head_on_stack()}
before first use and \apiDk{destroy_rcu_head_on_stack()} after last use,
...
Stall-warning control is provided by \apiDk{rcu_cpu_stall_reset()}, which
...
Callback checking is provided by \apiDk{rcu_head_init()} and
\apiDk{rcu_head_after_call_rcu()}.
...
\apiDk{rcu_read_lock_held()},
\apiDk{rcu_read_lock_bh_held()},
\apiDk{rcu_read_lock_sched_held()}, and
\apiDk{srcu_read_lock_held()},
...
to become quite ornate, \apiDk{rcu_is_watching()} returns \co{true} if
...
\apiDk{RCU_LOCKDEP_WARN()} emits a warning if lockdep is enabled and if
...
\apiDk{RCU_NONIDLE()} may be used to force RCU to watch when executing
...
Finally,  \apiDk{rcu_sleep_check()} emits a warning if invoked within
...
\apiDk{hlist_for_each_entry_rcu()}, etc.
Similarly, the RCU list-mutation primitives include
\apiDk{list_add_rcu()}, \apiDk{hlist_del_rcu()}, etc.
