...
Of course, \IXhpl{sequence}{lock} provides no pointer-traversal protection,
...
are traversing, the read-side overhead of \IXpl{reference counter} might
not be so important.
...
In 2010, \ppl{Keith}{Bostic} added hazard pointers to
WiredTiger~\cite{KeithBostic2010WiredTigerhazptr}.
MongoDB 3.0, released in 2015, included WiredTiger and thus hazard pointers.

In 2011, \ppl{Samy}{Al Bahra} added hazard pointers to the Concurrency Kit
library~\cite{SamyAlBahra2011ckhp}.

In 2014, \ppl{Maxim}{Khizhinsky} added hazard pointers to
libcds~\cite{MaximKhizhinsky2014libcdsHazptr}.

In 2015, \ppl{David}{Gwynne} introduced shared reference pointers, a form
...
In 2018, \ppl{Maged}{Michael} added hazard pointers to Facebook's Folly
...
In 2011, \ppl{Samy}{Al Bahra} added sequence locking to the Concurrency Kit
library~\cite{SamyAlBahra2011ckseqlock}.

\ppl{Paolo}{Bonzini} added a simple sequence-lock to the QEMU emulator in
2013~\cite{PaoloBonzini2013QEMUseqlock}.

\ppl{Alexis}{Menard} abstracted a sequence-lock implementation in Chromium
...
The Linux kernel adopted \ppl{Dipankar}{Sarma}'s implementation of RCU in
...
In 2011, \ppl{Samy}{Al Bahra} added epochs
...
\ppl{Paolo}{Bonzini} added RCU support to the QEMU emulator in 2015 via a
friendly fork of the userspace RCU
library~\cite{MikeDay2013RCUqemu,PaoloBonzini2013QEMURCU}.

In 2015, \ppl{Maxim}{Khizhinsky} added RCU to
libcds~\cite{MaxKhiszinsky2015C++RCU}.

\ppl{Mindaugas}{Rasiukevicius} implemented libqsbr in 2016, which features
QSBR and \IXacrf{ebr}~\cite{MindaugasRasiukevicius2016libqsbr},
both of which are types of implementations of RCU\@.

\pplsur{Harshal}{Sheth} et al.~\cite{HarshalSheth2016goRCU}
...
\ppl{Matt}{Klein} describes an RCU-like mechanism that is used in the Envoy
Proxy~\cite{MattKlein2017EnvoyRCU}.

\ppl{Honnappa}{Nagarahalli} added an RCU library to the Data Plane Development
Kit (DPDK) in 2018~\cite{HonnappaNagarahalli2018dpdkRCU}.

\ppl{Stjepan}{Glavina} merged an epoch-based RCU implementation into the
