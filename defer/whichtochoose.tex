...
%		  RC	  HP	  SL	  RCU \\
	\hline
	\rrc{1in}{2pt}{Existence Guarantees}
		& Complex
			& Yes
				& No
					& Yes \\
	\hline
	\rrc{1in}{-5pt}{Updates and Readers Progress Concurrently}
		& Yes
			& Yes
				& No
					& Yes \\
	\hline
	\rrc{1in}{2pt}{Contention Among Readers}
		& High
			& None
				& None
					& None \\
	\hline
	\rrc{1in}{-5pt}{Reader Per-Critical-Section Overhead}
		& N/A
			& N/A
				& Two \co{smp_mb()}
					&  \rrc{1in}{-5pt}{Ranges from none to two \co{smp_mb()}} \\
	\hline
	\rrc{1in}{2pt}{Reader Per-Object Traversal Overhead}
		& \rrc{1.2in}{2pt}{Read-modify-write atomic operations, memory-barrier
		  instructions, and cache misses}
			& \co{smp_mb()}
				& None, but unsafe
					& \rrc{1in}{2pt}{None (volatile accesses)} \\
	\hline
	\rrc{1in}{2pt}{Reader Forward Progress Guarantee}
		& Lock free
			& Lock free
				& Blocking
					& Bounded wait free \\
	\hline
	\rrc{1in}{2pt}{Reader Reference Acquisition}
		& \rrc{1.2in}{2pt}{Can fail (conditional)}
			& \rrc{0.8in}{2pt}{Can fail (conditional)}
				& Unsafe
					& \rrc{1in}{2pt}{Cannot fail (unconditional)} \\
	\hline
	Memory Footprint
		& Bounded
			& Bounded
				& Bounded
					& Unbounded \\
	\hline
	\rrc{1in}{2pt}{Reclamation Forward Progress}
		& Lock free
			& Lock free
				& N/A
					& Blocking \\
	\hline
	\rrc{1in}{2pt}{Automatic Reclamation}
		& Yes
			& No
				& N/A
					& No \\
