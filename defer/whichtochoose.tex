...
\IX{hazard pointer}s, \IXh{sequence}{lock}ing, and RCU\@.
...
if you need \IX{existence guarantee}s for linked
...
RCU \IX{grace period}s can also be short, which limits the memory footprint.
...
In 2018, \pplalt{Maged Michael}{Michael, Maged M.} added hazard pointers
to Facebook's Folly library~\cite{MagedMichael2018FollyHazptr}, where
it is used heavily.
