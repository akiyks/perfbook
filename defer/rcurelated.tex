...
The known first mention of anything resembling \acr{rcu} took the form of a bug
report from
...
The first known non-bug-report mention of anything resembling \acr{rcu} appeared
...
However, in the mid 2010s, there was a welcome upsurge in \acr{rcu} research
and development across a number of communities and
institutions~\cite{FransKaashoek2015ParallelOSHistory}.
Section~\ref{sec:defer:RCU Uses} describes uses of \acr{rcu},
Section~\ref{sec:defer:RCU Implementations} describes \acr{rcu} implementations
...
of MIT created an \acr{rcu}-optimized balanced binary tree
...
Harvard University created another \acr{rcu}-protected tree named
Masstree~\cite{Mao:2012:CCF:2168836.2168855} that combines ideas from B+
trees and tries.
Although this tree is about 2.5x slower than an \acr{rcu}-protected hash table,
...
approach~\cite{MayaArbel2014RCUtree} to an \acr{rcu}-protected search tree that,
...
however, Arbel and Attiya instead created an \acr{rcu} variant that reduces
low-end grace-period latency.
Of course, nothing comes for free, and this \acr{rcu} variant appears to hit
...
experimenting with alternative \acr{rcu} implementations.

\subsubsection{RCU Implementations}
\label{sec:defer:RCU Implementations}

\ppl{Mathieu}{Desnoyers} created a
\IXalthacrrcu{user-space \acr{rcu}}{User-space RCU}{user-space} for use in
...
working on \acr{rcu} implementations, including dissertations by
...
\acr{rcu}-like mechanisms are also finding their way into Java.
\pplsur{KC}{Sivaramakrishnan} et al.~\cite{Sivaramakrishnan:2012:ERB:2258996.2259005}
use an \acr{rcu}-like mechanism to eliminate the read barriers that are
...
created a specialized variant of \acr{rcu} that they used for an optimized
...
\ppl{Mike}{Ash} posted~\cite{MikeAsh2015Apple} a description of an \acr{rcu}-like
...
produced ``\IXr{Poor Man's RCU}'', which, despite using a pair of reader-writer
...
produced ``\IXr{Predicate RCU}'', which works hard to reduce grace-period
...
produced an \acr{rcu}-like mechanism that can be thought of as
...
This appears to be the first academic \acr{rcu}-related work to
...
\acr{rcu}, which was accepted into v4.4.

\ppl{Geoff}{Romer} and \ppl{Andrew}{Hunter} (both at Google) proposed
a cell-based API for \acr{rcu}
...
Researchers at Stony Brook University have produced an \acr{rcu}-aware data-race
...
semantics of \acr{rcu} in terms of separation logic, and have continued with
...
produced a manual formal proof of correctness of the \IXacr{qsbr}
variant of userspace
\acr{rcu}~\cite{MathieuDesnoyers2009URCU,MathieuDesnoyers2012URCU}.
...
used the \IXacr{cbmc}~\cite{EdmundClarke2004CBMC}
to produce a mechanical proof of correctness of a significant portion
of Linux-kernel \IXalthacrrcu{Tree \acr{rcu}}{Tree RCU}{tree}.
\ppl{Lance}{Roy}~\cite{LanceRoy2017CBMC-SRCU} used \acr{cbmc} to produce a similar
proof of correctness for a significant portion of Linux-kernel
\IXalthacrrcu{\acr{srcu}}{Sleepable RCU (SRCU)}%
{sleepable (SRCU)}~\cite{PaulEMcKenney2006c}.
...
portion of Linux-kernel \IXalthacrrcu{Tree \acr{rcu}}{Tree RCU}{tree}.

None of these efforts located any bugs other than bugs injected into
\acr{rcu} specifically to test the verification tools.
In contrast,
...
automatically mutated Linux-kernel \acr{rcu}'s source code to test the
coverage of the \co{rcutorture} test suite.
The effort found several holes in this suite's coverage, one of which
was hiding a real bug (since fixed) in
\IXalthacrrcu{Tiny \acr{rcu}}{Tiny RCU}{tiny}.
