...
be invoked from hardware \IXalt{interrupt}{IRQ} handlers or from
\IXalt{non-maskable interrupt (NMI)}{NMI} handlers.
...
\IXalt{non-maskable interrupt (NMI)}{NMI}
...
\co{synchronize_sched()} to ensure that all in-flight \IX{NMI}
...
which is called from within an \IX{NMI} handler.
...
including \IX{NMI} handlers, to complete.
