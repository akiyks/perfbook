...
In contrast, \co{cbmc} and \IX{Nidhugg} can input C programs of reasonable
...
The \co{cbmc} and \IX{Nidhugg} tools provide some ability to select
...
with \IXalth{sequential consistency}{Sequential consistency}{Memory consistency!sequential}.
...
In contrast, Promela/\co{spin}, \co{cbmc}, and \IX{Nidhugg} are designed for
...
both \co{cbmc} and \IX{Nidhugg} were appled to Linux-kernel RCU.
...
code, similar to Promela/spin's, as does \IX{Nidhugg}.
...
And in fact formal-verification tools such as \co{cbmc} and \IX{Nidhugg}
...
\IX{Nidhugg} also inputs C code directly, so its first cell is also blue.
