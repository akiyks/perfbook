...
As of 2020, \IXacr{htm} has been available for many
...
This section makes a first attempt to find \acr{htm}'s place in the parallel
...
but focused on \acr{htm} rather than \IXacr{tm} as a whole.\footnote{
...
\IXalth{Fully associative \emph{victim caches}}{fully associative cache}
{cache}{fully associative} would alleviate the associativity
...
use locking, particularly during the time before \acr{htm} is readily available
on all relevant
...
The literature on real-time use of \acr{htm} is quite sparse, perhaps because
...
same way that \IXacr{cas} and load-linked/store-conditional
...
currently available \IXacr{htm} implementations refuse to make any
sort of forward-progress guarantee, which means that \acr{htm} cannot be
used to avoid deadlock on those systems.\footnote{
	\Acr{htm} might well be used to reduce the probability of deadlock,
...
\QuickQuiz{
	...
	And perhaps there will be additional \IXacr{htm} progress over time.
}\QuickQuizEnd
...
\Acr{rcu} further provides contention-free wait-free read-side
...
The fact that \IXacr{htm} transactions must have fallbacks might in some cases
...
And as of late 2012, the IBM Mainframe announced an \IXacr{htm}
implementation that includes \emph{constrained transactions}
in addition to the usual best-effort \acr{htm}
...
usefulness of \IXacr{htm}, thereby increasing the need for CPUs to either
