...
\IXalth{}{Hardware transactional memory (HTM)|(}{Transactional memory (TM)!hardware (HTM)|(}%
...
is termed that cache's
\emph{\IXalt{associativity}{Cache associativity}}.
...
would alleviate the associativity
...
\IXalth{\emph{Unbounded transactional memory} (UTM)}
{Unbounded transactional memory (UTM)}{Transactional memory (TM)!unbounded (UTM)}
...
same way that compare-and-swap (\IX{CAS}) and load-linked/store-conditional
