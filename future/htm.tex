...
As of 2021, \IXacrfst{htm} has been available for many
...
This section makes an attempt to identify \acr{htm}'s place in the parallel
programmer's toolbox.

From a conceptual viewpoint, \IXacr{htm} uses processor caches and speculative
...
\IXAcr{htm} has a number of important benefits, including automatic
...
However, it always pays to read the fine print, and \IXacr{htm} is no exception.
A major point of this section is determining under what conditions \acr{htm}'s
benefits outweigh the complications hidden in its fine print.
To this end, \cref{sec:future:HTM Benefits WRT Locking}
describes \acr{htm}'s benefits and
...
\IXacr{htm}'s weaknesses with respect to the combination of synchronization
primitives used in the Linux kernel (and in many user-space applications).
\Cref{sec:future:Where Does HTM Best Fit In?} looks at where \acr{htm}
...
greatly increase \acr{htm}'s scope and appeal.
...
The primary benefits of \IXacr{htm} are
...
I break from \acr{tm} tradition by not listing ease of use separately
for two reasons.
First, ease of use should stem from \acr{htm}'s primary benefits,
...
In contrast, \IXacr{htm} synchronizes by using the CPU's cache, avoiding the need
for a separate synchronization data structure and resultant cache misses.
\Acr{htm}'s advantage is greatest in cases where a lock data structure is
placed in a separate cache line, in which case, converting a given
critical section to an \acr{htm} transaction can reduce that critical section's
...
\IXacr{htm} is therefore a very real possibility for large non-partitionable
...
Some evidence of \IXacr{htm}'s practical value has been demonstrated in a number
...
However, \IXacr{htm} also has some very real shortcomings, which will be discussed
in the next section.

\subsection{HTM Weaknesses WRT Locking}
\label{sec:future:HTM Weaknesses WRT Locking}

The concept of \IXacr{htm} is quite simple: A group of accesses and updates to
...
The transaction-size limitations of current \IXacr{htm} implementations
...
Although \IX{cache associativity} varies, the eight-way associativity of
...
That said, \IXacr{htm} victim caches for unmodified cache lines can be quite
...
\IXAcr{utm} schemes~\cite{CScottAnanian2006,KevinEMoore2006}
...
Finally, the ``unbounded'' aspect of \acr{utm} assumes that all of DRAM
...
fallback mechanism for \IXacr{htm}\@.

However, to the best of my knowledge, with the exception of abbreviating
representation of \IXacr{tm} read sets, currently available systems do not
...
However, for \IXacr{htm} to work effectively, the probability of conflict must
...
use locking, particularly until such time as \acr{htm} is readily available
on all relevant
...
breakpoints (no single stepping in the debugger for \gls{htm} transactions!!!).
...
Of course, aborts and rollbacks raise the question of whether \IXacr{htm} can
be useful for hard real-time systems.
Do the performance benefits of \acr{htm} outweigh the costs of the aborts
...
The literature on real-time use of \acr{htm} is quite sparse, perhaps
because there are more than enough problems in making \acr{htm} work well in
non-real-time environments.

Because current \IXacr{htm} implementations might deterministically abort a
...
It is clear that users of \IXacr{htm} must put considerable validation effort
...
\acr{htm} hardware are any less daunting.
...
same way that \IXacrf{cas} and load-linked/store-conditional
...
currently available \acr{htm} implementations refuse to make any
sort of forward-progress guarantee.
As noted earlier, \acr{htm} therefore cannot be used to avoid deadlock on
those systems.
Hopefully future implementations of \acr{htm} will provide some sort of
forward-progress guarantees.
Until that time, \acr{htm} must be used with extreme caution in real-time
applications.
...
It will be interesting to see if \acr{htm} forward-progress guarantees migrate
from the mainframe to commodity CPU families.

\subsubsection{Irrevocable Operations}
\label{sec:future:Irrevocable Operations}

Another consequence of aborts and rollbacks is that \IXacr{htm} transactions
cannot accommodate irrevocable operations.
Current \acr{htm} implementations typically enforce this limitation by
...
Note that buffered I/O can be accommodated by \IXacr{htm} transactions as
...
Although \IXacr{htm} can in many cases be used as a drop-in replacement for locking
(hence the name \IXacr{tle}~\cite{DaveDice2008TransactLockElision}),
...
lost.
In other words, \acrl{tle} preserves the data-protection
...
\QuickQuizB{
	...
	will operate correctly in the presence of \IXacrl{tle}.
	Although I have not seen a proof of this statement, there
	is a straightforward rationale for this claim.
	The main idea is that in a strongly atomic \acr{htm} implementation,
...
%
\QuickQuizM{
	Can't \IXacrl{tle} handle locking's
	...
	Using \acrl{tle} in such a case would be
...
Unfortunately, this arrangement can break down in presence of
\IXacrl{tle}.
...
differences, application of \IXacr{htm}-based lock elision to large programs
should be undertaken with caution.
That said, where it does apply, \acr{htm}-based lock elision can eliminate
...
Although it seems likely that \IXacr{htm} will have compelling use cases,
...
\Acr{htm}'s current situation relative to locking is summarized in
\cref{tab:future:Comparison of Locking and HTM}.
As can be seen, although the current state of \acr{htm} alleviates some
...
These shortcomings are acknowledged by leaders in the \acr{tm}
...
and \IXacr{rcu}~\cite{McKenney98,McKenney01a,ThomasEHart2007a,PaulEMcKenney2012ELCbattery}.
...
non-blocking data structures, hazard pointers, and \IXacr{rcu} to avoid some
of the shortcomings of locking.
For example, deadlock can be avoided in many cases by using reference
counts, hazard pointers, or \acr{rcu} to protect data structures,
...
\Acr{rcu} further provides contention-free bounded wait-free read-side
...
\cref{tab:future:Comparison of Locking and HTM}
results in the updated comparison between augmented locking and \IXacr{htm}
shown in
...
\item	Read-side mechanisms such as hazard pointers and \IXacr{rcu} can operate
	efficiently on non-partitionable data.
\item	Hazard pointers and \acr{rcu} do not contend with each other or with
	updaters, allowing excellent performance and scalability for
	read-mostly workloads.
\item	Hazard pointers and \acr{rcu} provide forward-progress guarantees
	(lock freedom and bounded wait-freedom, respectively).
\item	Privatization operations for hazard pointers and \acr{rcu} are
	straightforward.
\end{enumerate}
...
Of course, it is also possible to augment \IXacr{htm},
as discussed in the next section.

\subsection{Where Does HTM Best Fit In?}
\label{sec:future:Where Does HTM Best Fit In?}

Although it will likely be some time before \IXacr{htm}'s area of applicability
can be as crisply delineated as that shown for \IXacr{rcu} in
...
\IXAcr{htm} seems best suited to update-heavy workloads involving relatively
small changes to disparate portions of relatively large in-memory
data structures running on large multiprocessors,
as this meets the size restrictions of current \acr{htm} implementations while
...
Use of locking in conjunction with \IXacr{htm} seems likely to overcome \acr{htm}'s
difficulties with irrevocable operations, while use of \IXacr{rcu} or
hazard pointers might alleviate \acr{htm}'s transaction-size limitations
for read-only operations that traverse large fractions of the data
structure~\cite{SeongJaePark2020HTMRCUlock}.
Current \acr{htm} implementations unconditionally abort an update transaction
that conflicts with an \acr{rcu} or hazard-pointer reader, but perhaps future
\acr{htm} implementations will interoperate more smoothly with these
synchronization mechanisms.
In the meantime, the probability of an update conflicting with a
large \acr{rcu} or hazard-pointer read-side critical section should be
...
read-only transaction.\footnote{
	...
	Nevertheless, \acr{htm} has in fact realized the ease-of-use promise
	of \acr{tm}, albeit for black-hat attacks on the Linux kernel's
	address-space randomization defense
	mechanism~\cite{YeongjinJang2016TSXbreakKASLR,Jang:2016:BKA:2976749.2978321}.}
Nevertheless, it is quite possible that a steady stream of \acr{rcu} or
...
The fact that \acr{htm} transactions must have fallbacks might in some cases
force static partitionability of data structures back onto \acr{htm}\@.
This limitation might be alleviated if future \acr{htm} implementations
provide forward-progress guarantees, which might eliminate the need
for fallback code in some cases, which in turn might allow \acr{htm} to
be used efficiently in situations with higher conflict probabilities.

In short, although \IXacr{htm} is likely to have important uses and applications,
...
Game changers that could greatly increase the need for \IXacr{htm} include
the following:
...
current \IXacr{htm} implementations lack forward-progress guarantees, which requires
that fallback software is available to handle \acr{htm} failures.
Of course, it is easy to demand guarantees, but not always easy
to provide them.
In the case of \acr{htm}, obstacles to guarantees can include cache size and
associativity, TLB size and associativity, transaction duration and
interrupt frequency, and scheduler implementation.

Cache size and \IXalt{associativity}{cache associativity} was discussed in
\cref{sec:future:Transaction-Size Limitations},
along with some research intended to work around current limitations.
However, \IXacr{htm} forward-progress guarantees would
come with size limits, large though these limits might one day be.
So why don't current \acr{htm} implementations provide forward-progress
...
\IXacr{htm} implementations, it is necessary that the execution duration of
...
evidenced by the large number of \IXacr{htm} conflict-resolution policies
...
So there are significant obstacles to \IXacr{htm} vendors offering forward-progress
guarantees.
However, the impact of any of them doing so would be enormous.
It would mean that \acr{htm} transactions would no longer need software
fallbacks, which would mean that \acr{htm} could finally deliver on the
\IXacr{tm} promise of deadlock elimination.

However, in late 2012, the IBM Mainframe announced an \acr{htm} implementation
...
Constrained \IXacr{htm} therefore seems likely to become an important tool in
the parallel programmer's toolbox.

Note that these forward-progress guarantees need not be absolute.
For example, suppose that a use of \IXacr{htm} uses a global lock as fallback.
...
then if \acr{htm} rollbacks are sufficiently infrequent, the global lock
...
\IXacr{htm} implementations use approximation techniques to support extremely
large \acr{htm} read sets~\cite{RaviRajwar2012TSX}.
For another example, \Power{8} \acr{htm} supports suspended transations, which
...
increases the usefulness of \acr{htm}, thereby increasing the need for CPUs
...
the processor (slow!), substituting \IXacr{stm} for \IXacr{htm} (slow and slightly
different semantics!),
playback techniques using repeated retries to emulate forward
progress (strange failure modes!), and
full support of debugging \acr{htm} transactions (complex!).

Should one of the \IXacr{htm} vendors produce an \acr{htm} system that allows
straightforward use of classical debugging techniques within
transactions, including breakpoints, single stepping, and
print statements, this will make \acr{htm} much more compelling.
...
with \acr{htm}~\cite{RobertOCallahan2020DebuggingHTM}.

\subsubsection{Weak Atomicity}
\label{sec:future:Weak Atomicity}

Given that \IXacr{htm} is likely to face some sort of size limitations for the
foreseeable future, it will be necessary for \acr{htm} to interoperate
smoothly with other mechanisms.
\Acr{htm}'s interoperability with read-mostly mechanisms such as hazard pointers
and \IXacr{rcu} would be improved if extra-transactional reads did not
...
In this way, hazard pointers and \acr{rcu} could be used to allow \acr{htm} to handle
...
is to use \IXacr{rcu} for read-only traversals and \IXacr{htm}
...
when they combined \acr{rcu} with \IXacr{stm}\@.
...
Although current \IXacr{htm} implementations have delivered real performance
...
\IXacr{htm} work well with the many other types of synchronization mechanisms,
as noted earlier~\cite{McKenney2007PLOSTM,PaulEMcKenney2010OSRGrassGreener}.
Although there has been some work using \acr{htm} with
\IXacr{rcu}~\cite{Siakavaras2017CombiningHA,DimitriosSiakavaras2020RCU-HTM-B+Trees,ChristinaGiannoula2018HTM-RCU-graphcoloring,SeongJaePark2020HTMRCUlock},
there has been little evidence of progress towards \acr{htm} work better with
\acr{rcu} and with other deferred-reclamation mechanisms.

In short, current \IXacr{htm} implementations appear to be welcome and useful
