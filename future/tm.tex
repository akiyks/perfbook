...
The idea of supporting memory-based transactions, or \IXacrfst{tm}, in hardware
...
But by the turn of the century, \IXacr{tm} started receiving
...
The basic idea behind \IXacr{tm} is to execute a section of
code atomically, so that other threads see no intermediate state.
As such, the semantics of \acr{tm} could be implemented
...
and scalability.
Much of the complexity inherent in \acr{tm} implementations, whether hardware
...
size decreases, \acr{tm} might become quite attractive for small memory-based
operations,
...
\item	Prohibit I/O within transactions, so that any attempt to execute
	...
	This approach seems to be the conventional \IXacr{tm} approach for
	unbuffered I/O, but requires that \acr{tm} interoperate with other
...
\item	Permit only one special
	...
	operations.\footnote{
		...
		To see the problem, think through what \acr{tm} should do
...
I/O operations are a well-known weakness of \IXacr{tm}, and it is not clear
...
Here are some options available to \IXacr{tm}:
...
\item	Prohibit RPC within transactions, so that any attempt to execute
	...
	transactions.
	This approach does work, but will require \acr{tm} to
...
\item	Extend the transactional substrate to include the RPC server as
	...
	techniques, given that memory-based \IXacr{tm} has no slow disk drives
...
As noted in the prior section, I/O is a known weakness of \acr{tm}, and RPC
...
face of \acr{tm}'s atomicity property, but this sort of thing is arguably what
...
So, what can \IXacr{tm} do about time delays within transactions?
...
\item	Let the time delays execute normally.
	Unfortunately, some \acr{tm} implementations publish modifications only
	at commit time, which could defeat the purpose of the time delay.
\end{enumerate}

It is not clear that there is a single correct answer.
\IXacr{tm} implementations featuring weak atomicity that publish changes
...
How could a similar persistent functionality be provided for \IXacr{tm}?
...
What might \IXacr{tm} do about thread spawning within a transaction?
...
\item	Permit \co{pthread_create()} to be executed within a
	...
	posited \acr{tm} implementations, but seems to be a trap for the unwary.
...
\item	Extend the transaction to cover the parent and all child threads.
	...
	The answers for \acr{tm} are left as an exercise for the reader.
\end{enumerate}

Given that parallel execution of transactions is commonplace in the
database world, it is perhaps surprising that current \IXacr{tm} proposals do
not provide for it.
...
and there are rumors that other \acr{tm} researchers are investigating
...
\item	Disallow \co{exec()} within transactions, so that the enclosing
	...
	This is well defined, but clearly requires non-\IXacr{tm} synchronization
...
\item	Disallow \co{exec()} within transactions, with the compiler
	...
	There is a draft specification for \acr{tm} in C++ that takes
	...
	at runtime, but again requires non-\acr{tm} synchronization primitives
...
obstacle to universal \IXacr{tm} applicability, as it is not completely clear
...
to still be considerable uncertainty in the case of \IXacr{tm}\@.
In fact, different implementations of TM seem to have different restrictions.

So what can \acr{tm} do about dynamically linked and loaded library functions?
...
Options for part (b), the inability to detect \IXacr{tm}-unfriendly operations
in a not-yet-loaded function, possibilities include the following:

\begin{enumerate}
\item	Just execute the code: if there are any \acr{tm}-unfriendly operations
...
I/O operations are of course a known weakness of \IXacr{tm}, and dynamic linking
and loading can be thought of as yet another special case of I/O\@.
Nevertheless, the proponents of \acr{tm} must either solve this problem, or
resign themselves to a world where \acr{tm} is but one tool of several in the
parallel programmer's toolbox.
(To be fair, a number of \acr{tm} proponents have long since resigned themselves
to a world containing more than just \acr{tm}.)
...
\item	The effect of memory-mapping operations that overlap the memory
	...
	\IXacr{tm} conflict manager, which might dynamically determine whether
...
\item	Use only hardware \IXacr{tm} implementations that are capable of
...
\item	Use only software \acr{tm} implementations, which are
	(very roughly speaking) more tolerant of exceptions than are
	the simpler of the hardware TM (\glsunset{htm}\IXacr{htm}) implementations.
	Of course, software TM (\glsunset{stm}\IXacr{stm}) tends to have higher
	overhead than \acr{htm},
	so this approach may not be acceptable in all situations.
...
can arise, depending on implementation details of the \IXacr{tm} system.
...
\begin{enumerate}
\item	Use only locking-friendly \IXacr{tm} implementations.
	...
	transactions.
\item	Use \acr{tm} only ``in the small'' when introducing \acr{tm} to lock-based
	programs, thereby accommodating the limitations of
	locking-friendly \acr{tm} implementations.
...
\item	Use \acr{tm} strictly as an optimization in lock-based systems, as was
...
The fact that there could possibly be a problem interfacing \IXacr{tm} and locking
...
\item	Use \IXacr{tm} only ``in the small'' when introducing \acr{tm} to lock-based
...
\item	Use \acr{tm} strictly as an optimization in lock-based systems, as was
	...
	and as has been done by more recent work using \acr{tm} to elide
...
combining \IXacr{tm} with reader-writer locking, as there in fact were with
exclusive locking.
...
Similar issues and possible resolutions arise when combining \IXacr{tm} with
...
This means that any \acr{tm} implementation that chooses not to surmount each
...
the group to take on the challenge of RCU/\acr{tm}
interoperability~\cite{ChistopherJRossbach2007a}.
Because they applied \acr{tm} to the Linux 2.6 kernel, which uses RCU, they
had no choice but to integrate \acr{tm} and RCU, with \acr{tm} taking the place of
...
\IXacr{htm} and RCU to search trees~\cite{Siakavaras2017CombiningHA,DimitriosSiakavaras2020RCU-HTM-B+Trees},
\ppl{Christina}{Giannoula} et al.~have used \acr{htm} and RCU to color
graphs~\cite{ChristinaGiannoula2018HTM-RCU-graphcoloring},
and
\ppl{SeongJae}{Park} et al.~have used \acr{htm} and RCU to optimize high-contention
...
underlying atomicity properties of \IXacr{tm}, although the \Power{8} CPU family's
suspended-transaction facility~\cite{Le:2015:TMS:3266491.3266500} makes
it an exception to this rule.

So how should \IXacr{tm}-based updates interact with concurrent RCU readers?
Some possibilities are as follows:

\begin{enumerate}
\item	RCU readers abort concurrent conflicting \acr{tm} updates.
	...
	in system hangs.
	In addition, not all \acr{tm} implementations offer the strong atomicity
	required to implement this approach, and for good reasons.
\item	RCU readers that run concurrently with conflicting \acr{tm} updates
	...
	However, not all \acr{tm} implementations can provide timely access
	to old values of variables that have been tentatively updated
	by an in-flight transaction.
	In particular, log-based \acr{tm} implementations that maintain
	old values in the log (thus providing excellent \acr{tm} commit
	...
	of \acr{tm} implementations, though performance might still be an issue.
	Nevertheless, there are popular \acr{tm} implementations that have
...
\item	If an RCU reader executes an access that conflicts with an
	...
	transactions.
	In addition, not all \IXacr{tm} implementations are capable of delaying
...
\item	RCU readers are converted to transactions.
	...
	any \acr{tm} implementation, but it also imposes \acr{tm}'s rollbacks on RCU
	...
	the \acr{tm} implementation in question is incapable of handling.
...
\item	Many update-side uses of RCU modify a single pointer to publish
	...
	Unfortunately, not all \IXacr{tm} implementations respect memory barriers
...
\item	Prohibit use of \acr{tm} in RCU updates.
	This is guaranteed to work, but restricts use of \acr{tm}.
\end{enumerate}
...
scaling \IXacr{stm} implementations make use of RCU-like techniques
...
\QuickQuiz{
	...
	the inherent limitations of \IXacr{stm} noted by \ppl{Hagit}{Attiya} et
...
from the proponents of \IXacr{tm}, with the concept of weak
...
The obstacles to universal \IXacr{tm} adoption lead to the following
conclusions:

\begin{enumerate}
\item	One interesting property of \acr{tm} is the fact that transactions are
	subject to rollback and retry.
	This property underlies \acr{tm}'s difficulties with irreversible
...
\item	Another interesting property of \acr{tm}, noted by
	Shpeisman et al.~\cite{TatianaShpeisman2009CppTM}, is that \acr{tm}
	intertwines the synchronization with the data it protects.
	This property underlies \acr{tm}'s issues with I/O, memory-mapping
...
\item	One of the stated goals of many workers in the \acr{tm} area is to
	ease parallelization of large sequential programs.
	As such, individual transactions are commonly expected to
	execute serially, which might do much to explain \IXacr{tm}'s issues
	with multithreaded transactions.
\end{enumerate}

\QuickQuiz{
	Given things like \co{spin_trylock()}, how does it make any
	sense at all to claim that \IXacr{tm} introduces the concept of failure???
}\QuickQuizAnswer{
	...
	When using \acr{tm}, the only failure-free choice is the irrevocable
	transaction, which is not used in the common case.
	In fact, the irrevocable transaction is not even available
	in all \acr{tm} implementations.
}\QuickQuizEnd

What should \IXacr{tm} researchers and developers do about all of this?

One approach is to focus on \acr{tm} in the small, focusing on small
...
\acr{tm}-locking approach~\cite{VPankratius2011TMvsLockingProductivity}.
Sun took the small-transaction approach with its Rock research
CPU~\cite{DaveDice2009ASPLOSRockHTM}.
Some \acr{tm} researchers seem to agree with these two small-is-beautiful
approaches~\cite{JMStone93}, others have much higher hopes for \acr{tm},
and yet others
hint that high \acr{tm} aspirations might be \acr{tm}'s worst
enemy~\cite[Section 6]{Attiya:2010:ICT:1835698.1835699}.
It is nonetheless quite possible that \acr{tm} will be able to take on larger
problems, and this section has listed a few of the issues that must be
resolved if \acr{tm} is to achieve this lofty goal.

Of course, everyone involved should treat this as a learning experience.
It would seem that \IXacr{tm} researchers have great deal to learn from
...
\QuickQuiz{
	What is to learn?
	Why not just use \IXacr{tm} for memory-based data structures and locking
	...
	incandescent \acr{tm} marketing hype back.

	In the year 2021, \acr{tm} still has significant proving to do,
	even with the advent of \IXacr{htm}, which is covered in the
	upcoming
...
fig:future:The STM Reality: Realtime Response}.\footnote{
	Recent academic work-in-progress has investigated lock-based \acr{stm}
	...
	that real-time hybrid \acr{stm}/\IXacr{htm} systems must choose between fast
...
Less fanciful \IXacr{stm} retrospectives are also
...
\IXacr{htm}, which are addressed in the following section.
