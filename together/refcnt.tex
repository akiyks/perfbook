...
there would be no need for the \IX{reference counter} in the first place.
...
\begin{enumerate}
\item	Simple counting with neither atomic operations, \IXpl{memory
	barrier}, nor alignment constraints (``$-$'').
...
\item	Atomic counting with a check combined with the atomic acquisition
	operation (``CA'').
\item	Simple counting with a check combined with \IXhpl{full}{memory barrier}
	(``M'').
\item	Atomic counting with a check combined with the atomic acquisition
...
atomic counting with \IXalth{acquisition memory barrier}
{acquire}{memory barrier} (``AM'') is described in
\cref{sec:together:Atomic Counting With Release Memory Barrier},
and
atomic counting with check and \IXh{release}{memory barrier} (``CAM'') is described in
...
pointers or exited their \IXrhpl{RCU read-side}{critical section}, respectively.
...
Atomic reference counting with \IXhpl{release}{memory barrier} is used by the
