...
\QuickQuiz{
	...
	\IXacrl{nbs}~\cite{MauriceHerlihy90a},
	which don't have critical sections?
}\QuickQuizAnswer{
	Although \acrl{nbs} can be very useful
	in some situations, it is no panacea, as discussed in
	\cref{sec:advsync:Non-Blocking Synchronization}.
	Also, \acrl{nbs} really does have
	critical sections, as noted by \ppl{Josh}{Triplett}.
	For example, in a non-blocking algorithm based on
	\IXacrl{cas} operations, the code starting at the
	initial load and continuing to the \acrl{cas}
