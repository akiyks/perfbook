...
Therefore, you should use \IXh{code}{locking} on programs that spend
...
Unfortunately, \IXh{code}{locking} is particularly prone to ``\IX{lock contention}'',
...
One solution to this problem, named ``\IXh{data}{locking}'', is described
in the next section.
...
You should use \IXh{data}{locking} when contention must
...
\IXh{data}{locking} helps promote harmony, as illustrated by
...
\IXh{Data}{locking} is often used for partitionable data structures such as
...
\QuickQuiz{
	...
	\item	Use \IXacrfst{tm}~\cite{Herlihy93a,DBLomet1977SIGSOFT,Shavit95},
		so that each reference and
		modification to the data structure in question is
		performed atomically.
		Although \acr{tm} has engendered much excitement in recent years,
		...
		For more on \acr{tm}, including ways to overcome some of its
...
\IX{Data ownership} partitions a given data structure over the threads
...
However, in situations where no sharing is required, \IX{data ownership}
...
Another important instance of \IX{data ownership} occurs when the data
...
involves an \IX{atomic} increment instruction, and that a computer system is
...
\QuickQuiz{
	...
	scalability, but by that point \IXacrpl{gpgpu} become quite attractive,
...
rather than the toy program's \IXh{code}{locking}.
...
\IXh{Code}{locking} can often be tolerated at this level, because this
