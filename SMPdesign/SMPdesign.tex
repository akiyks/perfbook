...
then $\lambda$ would be \num{1 000} transactions per second.
...
The value of $\mu$ is therefore about \num{40 000 000} atomic increments
per second.
...
CPU attempting an atomic increment every \num{2.5} microseconds,
...
than \num{1.0} on as few as 10 threads, and even the 1024-by-1024 matrix
...
\QuickQuiz{}
	...
	have an efficiency of less than \num{1.0}?
	Shouldn't all of the traces in
	Figure~\ref{fig:SMPdesign:Matrix Multiply Efficiency}
	have efficiency of exactly \num{1.0} when running on only one thread?
\QuickQuizAnswer{
