...
hide bugs, resulting in \emph{\IXpl{heisenbug}}, which are discussed in
Section~\ref{sec:debugging:Probability and Heisenbugs}
...
scripts were quite satisfied with a test run in which RCU \IXpl{grace period}
...
The Linux kernel's \IXr{lockdep}
...
access to data.
The \IXacrf{kcsan}~\cite{JonathanCorbet2019KCSAN}
...
\item	Write sequential plug-ins for a parallel framework,
	such as map-reduce, \IXacr{boinc}, or a web-application server.
...
To qualify as a full-fledged error, an \IXrh{RCU read-side}{critical section}
