...
of the threads.
Interestingly enough, \IX{data ownership} covers each of the ``big three''
...
\QuickQuiz{
	What form of \IX{data ownership} is extremely difficult
...
Concurrent counting (see \cref{chp:Counting}) uses \IX{data ownership} heavily,
but adds a twist.
...
For its own part, pure \IX{data ownership} is also both common and useful,
...
\item	Many parallel linear-algebra algorithms, especially those
	well-suited for \IXacrpl{gpgpu}.\footnote{
		But note that a great many other classes of applications
		have also been ported to
		\acrpl{gpgpu}~\cite{NormMatloff2017ParProcBook,AMD2020ROCm,NVidia2017GPGPU,NVidia2017GPGPU-university}.}
