...
So what possessed CPU designers to cause them to inflict \IXB{memory barrier}s
...
called ``\IX{cache line}s'', which are normally a power of two in size,
...
\IX{Cache-coherence protocol}s manage cache-line states so as to prevent
...
as they travel through the various single-line \IXh{direct-mapped}{cache}s
...
\item	CPU~1 can now load the value of~\qco{b},
	...
	to the next statement.
\item	CPU~1 executes the \co{assert(a == 1)}, but the cache line
	containing~\qco{a} is no longer in its cache.
...
\QuickQuiz{
	...
	both CPUs might drop the cache line containing the new value
	of~\qco{b}.
...
At that point, the CPU must once again wait for \IX{invalidation}s to complete
...
and that \qco{b}~is
owned by CPU~0 (MESI ``exclusive'' or ``modified'' state).
...
\item	CPU~1 receives this cache line, which contains a value of~1
	for~\qco{a}, so that the assertion does not trigger.
\end{sequence}
\end{fcvref}
...
Then CPU~0's assignments to~\qco{a} and~\qco{b} will appear in Node~0's cache
...
\QuickQuizB{
	...
	The assertion must ensure that the load of~\qco{e}
	precedes that of~\qco{a}.
