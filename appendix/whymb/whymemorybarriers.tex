...
\item	describe how \IXpl{cache-coherence protocol} ensure that CPUs agree
	on the value of each location in memory, and, finally,
\item	outline how store buffers and invalidate queues help
	caches and cache-coherence protocols achieve high performance.
\end{enumerate}
...
Such a \IX{cache miss} is termed a
``\IXalth{capacity miss}{capacity}{cache miss}'', because it is caused
...
this case) are collectively called the cache's ``\IXalt{geometry}{cache geometry}''.
...
``\IXalth{associativity miss}{associativity}{cache miss}''.
...
``\IXalth{communication miss}{communication}{cache miss}'', since it
...
caches.
These problems are prevented by ``cache-coherence protocols'',
...
four-state MESI \IX{cache-coherence protocol}.
...
\QuickQuizM{
	...
	tend to use so-called ``directory-based'' \IXpl{cache-coherence
	protocol} to avoid this and other problems.
}\QuickQuizEndM
%
\QuickQuizE{
	...
	the past few decades.  One answer is that the \IXpl{cache-coherence
	protocol} are quite simple, and therefore can be implemented
	...
	a rebuttal might note the appearance of HPC clusters and \acr{mpi}\@.
...
\item	[Transition (i):]
	Some other CPU does an \IX{atomic read-modify-write operation} on
...
\QuickQuiz{
	...
	real-world \IXpl{cache-coherence protocol} being much more complex than
...
\IXhpl{direct-mapped}{cache} in a four-CPU system.
...
\QuickQuiz{
	...
	latencies in multiprocessor \IXpl{cache-coherence protocol}, why
...
case may be) is fully shouldered by the \IX{cache-coherence protocol}.
...
\item	\IXAcrpl{ipi} that ignore cache coherence.

	This can be problematic if the \acr{ipi} reaches its destination
