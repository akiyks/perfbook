...
So what possessed CPU designers to cause them to inflict \IXpl{memory barrier}
on poor unsuspecting \IXacr{smp} software designers?
...
\item	outline how \IXpl{store buffer} and invalidate queues help
...
Such a cache miss is termed an ``\IXalthalt{associativity miss}
...
this process is termed a ``\IXalthalt{write miss}{write}{cache miss}''.
...
This type of cache miss is termed a ``\IXalthalt{communication miss}
...
\QuickQuizE{
	...
	the appearance of HPC clusters and \IXacr{mpi}\@.
...
With \IX{store forwarding} in place, item~\ref{item:app:whymb:Need Store Buffer}
...
To see the second complication, a violation of \IXh{global}{memory ordering},
...
Roughly speaking, a ``\IXh{read}{memory barrier}'' marks only the invalidate
queue and a ``\IXh{write}{memory barrier}'' marks only the store buffer,
...
single-threaded \IX{throughput} implied by the massive multi-threaded
scenarios.
