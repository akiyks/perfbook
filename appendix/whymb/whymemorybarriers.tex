...
\QuickQuiz{}
	...
	Table~\ref{tab:app:whymb:Memory Barrier Example 3}
	are in interrupt
	handlers, and that the CPU~2's line~9 runs at process level.
	Also suppose that interrupt handlers can run on any CPU.
	In other words, the above mentioned code can
	run on the same CPU.
	This means that the assertion in third column can be interrupted
	by the code in the interrupt handlers.
