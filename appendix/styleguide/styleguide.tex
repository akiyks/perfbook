...
\centering
\theverbbox
\caption{\LaTeX\ Source of Sample Code Snippet (Deprecated)}
\label{fig:app:styleguide:LaTeX Source of Sample Code Snippet (Deprecated)}
\end{figure}

\input{appendix/styleguide/samplecodesnippetfig}

The \LaTeX\ source of a sample code snippet is shown in
Figure~\ref{fig:app:styleguide:LaTeX Source of Sample Code Snippet (Deprecated)}
...
In Figure~\ref{fig:app:styleguide:LaTeX Source of Sample Code Snippet (Deprecated)},
line numbers are manually placed for ease of referencing them within
\LaTeX\ sources.

This was how code snippets were coded until recently.
Transition to the auto-numbering scheme of verbbox and
``listing'' environment has started recently. See
Section~\ref{sec:app:styleguide:Environment for Code Snippets}
for a sample \LaTeX\ source of a code snippet using the new scheme.
...
using the ``listing'' environment.

\input{appendix/styleguide/samplecodesnippetlst}

Its \LaTeX\ source is shown in
Listing~\ref{lst:app:styleguide:LaTeX Source of Sample Code Snippet (Current)}.
Here, the auto-numbering feature of verbbox is also enabled.
The \verb|\LstLineNo| macro specified in the option to verbbox is
defined in preamble of \path{perfbook.tex} as the following.
...
vertical lines in a table.\footnote{
  ...
  from experimenting it at the moment.
}
