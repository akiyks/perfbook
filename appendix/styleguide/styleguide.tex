...
The symbol of micro ($10^{-6}$) can be typeset easily by
the help of the ``siunitx'' \LaTeX\ package.
The symbol of ``microsecond'' can be typeset as follows:
...
The angular-degree symbol (\textdegree) does not require any space in front
of it.
NIST style guide clearly states so.

This convention can be followed easily by using commands from \co{siunitx},
\co{\\qty} and \co{\\degree}.
...
The NIST style guide treats the percent symbol (\unit{\percent}) in the
same way as SI unit symbols.

\begin{quote}
  ``50\pct\ possibility'', rather than ``50\% possibility''.
\end{quote}

Again, the \co{siunitx} package provides a macro, \co{\\percent}, which can
be used as follows if you want to mark it up explicitly:
...
On the other hand, \verb|\tco{}| can not handle~\qco{\%},
\qco{\{}, \qco{\}}, nor~\qco{\\} properly.
...
\IfSubcaptionGood{
  \input{appendix/styleguide/groupingfigure}
}{
  \input{appendix/styleguide/groupingfigure-fallback}
}

\begin{listing*}%
\caption{Message-Passing Litmus Test (subcaption)}%
\label{lst:app:styleguide:Message-Passing Litmus Test (subcaption)}%
{\scriptsize%
...
\centering
\hspace*{\fill}
\subcaptionbox{Not Enforcing Order\label{sublst:app:styleguide:Not Enforcing Order}}{
  \theverbbox
}
\hspace{\fill}
{\scriptsize%
...
\subcaptionbox{Enforcing Order\label{sublst:app:styleguide:Enforcing Order}}{
  \theverbbox
}\hspace*{\fill}%
\end{listing*}
...
can also be grouped together by using the \co{\\subcaptionbox} macro,
as shown in
\cref{lst:app:styleguide:Message-Passing Litmus Test (subcaption)}.

\IfFootmiscGood{
  \input{appendix/styleguide/groupinglisting}
}{
  \input{appendix/styleguide/groupinglisting-fallback}
}
