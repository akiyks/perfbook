...
The new scheme allows us to do that by the help of escaping feature
of the \co{fancyvrb} package.
And the sources of snippets can be extracted from code samples under
...
the type of source file by its suffix and handle the meta comands
...
It also forbids a couple of tokens to appear in comments.
...
Example of disallowed comments in litmus test is shown below:
...
exists (0:r1=0)  // comment after exists clause
\end{VerbatimN}
\end{linelabel}

To avoid parse error, meta commands in litmus tests are embedded
...
This section presents a few experimental tables
