...
Although the \qco{verbatim} environment is commonly used to include
listings, we are transitioning to a new scheme which uses
the \qco{fancyvrb} package for code snippets.

Three variants of environment customized for perfbook are defined
in the preamble as shown below:
...
The above snippet uses the \co{VarbatimU} environment.

We used to use the \qco{verbbox} environment provided
by the \qco{verbatimbox} package. The new scheme employing
the \co{fancyvrb} package enables us to embed labels on lines
within the source of code snippets.
And the source can be extracted from code samples under
the \path{CodeSamples} directory of perfbook source tree.
...
Labels to lines of \qco{printf} and \qco{return} can be referred
to as follows:

\begin{VerbatimU}
\begin{lineref}[ln:app:styleguide:samplecodesnippet:main]
Lines~\lnref{printf} and~\lnref{return} can be referred
to from text.
\end{lineref}
\end{VerbatimU}

Above code results in the following paragraph:
...
Listing~\ref{lst:app:styleguide:LaTeX Source of Sample Code Snippet (Current)}
can be extracted from source of
Listing~\ref{lst:app:styleguide:Source of Code Sample} by a perl script
\path{utilities/fcvextract.pl}. All the relevant rules to extract
the \LaTeX\ source are described as recipes in the top level \path{Makefile}
and a script to generate dependencies (\path{utilities/gen_snippet_d.pl}).
...
In Listing~\ref{lst:app:styleguide:Source of Code Sample}, meta commands
are placed as comments of C (C++ style) and the source can be compiled
as is.  \path{utilities/fcvextract.pl} distinguishes
the type of source file by the suffix and handle the meta comands
in comments accordingly.
...
placed and restriction on which characters and tokens can appear
in comments. (Tokens in comments might sound strange, but they do
have some restrictions.)

For example, the first token in a litmus test must be one of
\qco{C}, \qco{PPC}, \qco{X86}, or~\qco{LISA}, which indicates
the flavour of the test. This means any comment is not allowed
in the beginning of a litmus test.

Similarly, several tokens such as \qco{exists}, \qco{filter},
and~\qco{locations} indicate the end of body of litmus test.
Once one of them appears in a litmus test, any comment is not
allowed to appear thereafter.  These tokens keep the same meaning
even when they appear in comments!

The pair of characters \qco{\{} and \qco{\}} also have special
meaning in C flavour. They are used to seperate portions in a litmus test.
First pair of \qco{\{} and \qco{\}} encloses initialization part.
Comments in this part should be of the form \qco{(* ... *)}.

You can't use \qco{\{} and \qco{\}} in comments in litmus test, either.

\begin{listing*}[th]
\begin{linelabel}[ln:app:styleguide:Sample Source of Litmus Test]
\begin{VerbatimL}[tabsize=8]
C C-SB+o-o+o-o
//\begin[snippet][labelbase=ln:sample:labelbase,commandchars=\%\@\$]
...
\end{VerbatimL}
\end{linelabel}
\caption{Sample Source of Litmus Test with Meta Commands}
\label{lst:app:styleguide:Sample Source of Litmus Test}
\end{listing*}

As a workaround, special form of meta commands are embedded in a litmus test
as shown in Listing~\ref{lst:app:styleguide:Sample Source of Litmus Test}.

Listing~\ref{lst:app:styleguide:Sample Source of Litmus Test}
is converted to
Listing~\ref{lst:app:styleguide:Intermediate Source of Litmus Test}
by a script \path{utilities/reorder_ltms.pl}. It can be handled
...
\end{VerbatimL}
\end{linelabel}
\caption{Intermediate Source of Litmus Test with Meta Commands}
\label{lst:app:styleguide:Intermediate Source of Litmus Test}
\end{listing*}
