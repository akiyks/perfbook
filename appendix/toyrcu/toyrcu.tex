...
with read-side overhead ranging from about 100\,ns on a single Power5
...
Furthermore, the read-side overhead, though high at roughly 140\,ns,
remains at about 140\,ns regardless of the number of CPUs.
However, the update-side overhead ranges from about 600\,ns
...
with read-side overhead ranging from about 100\,ns on
...
\QuickQuiz{}
	How can the grace period possibly elapse in 40\,ns when
...
Figure~\ref{fig:app:toyrcu:RCU Implementation Using Single Global Reference Counter},
with the read-side primitives consuming about 150\,ns on a single
...
well, ranging from about 200\,ns on a single Power5 CPU to
...
115\,ns regardless of whether running on a single-CPU or a 64-CPU
Power5 system.
As noted above, the \co{synchronize_rcu()} primitive does not scale,
ranging in overhead from almost 1\,\micro s on a single Power5 CPU
up to almost 200\,\micro s on a 64-CPU system.
...
However, even if each grace period took only
1\,\micro s, the offending thread would need to be
...
115\,ns of overhead regardless of the number of CPUs.
The \co{synchronize_rcu()} primitive is still expensive,
ranging from about 1\,\micro s up to about 16\,\micro s.
This is nevertheless much cheaper than the roughly 200\,\micro s
...
roughly 63\,ns of overhead regardless of the number of
Power5 CPUs.
Updates incur more overhead, ranging from about 500\,ns on
...
roughly 65\,ns of overhead regardless of the number of
Power5 CPUs.
Updates again incur more overhead, ranging from about 600\,ns on
a single Power5 CPU to more than 100~\emph{microseconds} on 64
such CPUs.
...
The \co{synchronize_rcu()} overhead ranges from about 600\,ns
on a single-CPU Power5 system up to more than 100\,\micro s on
a 64-CPU system.
