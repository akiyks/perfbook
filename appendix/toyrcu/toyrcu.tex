...
present a series of
simple RCU implementations based on locking, \IXpl{reference counter},
...
(and released) the lock, it cannot return until all prior \IXrhpl{RCU read-side}
...
counter, then executes a \IX{memory barrier} to ensure that the
...
Debugging versions of this primitive would check
(before decrementing!) that these low-order bits were non-zero.
\end{fcvref}
