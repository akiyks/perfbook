...
correctly enforces \IX{memory ordering} in case of concurrent insertion.
...
\IXrh{RCU read-side}{critical section}, for example, the caller must invoke
...
latter's \IXhpl{read-side}{memory barrier} result in dead time within the core.
...
execution, greatly reducing \IXh{memory-barrier}{overhead} in the read-only case.
...
pair of pointer sets does impose some \IXh{memory}{overhead},
...
\item	They are significantly slower than the traditional spinlock
	primitives.
\item	They cannot participate in the \IXr{lockdep} deadlock detection
