...
\epigraph{Finding a way to live the simple life today is the most
	  complicated task.}
	 {\pplEPmdl{Henry A.}{Courtney}, updated}
...
Similar results can be achieved using \IX{hazard pointer}s
...
latter's read-side \IX{memory barrier}s result in dead time within the core.
...
As noted earlier, \pplsur{Erwin}{Schr\"odinger}
is surprised by the popularity of his
...
quite well.
However, sharding does not handle ``\IX{hot spot}s'' very well, with
...
\begin{listing}
\ebresizeverb{.98}{\input{CodeSamples/datastruct/hash/hash_resize@resize.fcv}}
