...
In the example, line~\lnref{type} identifies the type of system
(``ARM'' or ``PPC'')
and contains the title for the model. Line~\lnref{altname}
...
lines~\lnref{altname} and~\lnref{init:b} using the OCaml
(or Pascal) syntax of \nbco{(* *)}.

Lines~\lnref{init:b}-\lnref{init:e} give initial values for
all registers; each is of the form
...
Line~\lnref{procid} provides identifiers for the two processes,
so that the \co{0:r3=2}
on line~\lnref{init:0} could instead have been written \co{P0:r3=2}.
Line~\lnref{procid} is
...
\QuickQuiz{}
	\begin{lineref}[ln:formal:PPCMEM Litmus Test]
	Why does line~\lnref{reginit}
	of Listing~\ref{lst:formal:PPCMEM Litmus Test}
	initialize the registers?
	Why not instead initialize them on lines~\lnref{init:0}
	and~\lnref{init:1}?
	\end{lineref}
\QuickQuizAnswer{
	Either way works.
...
\begin{lineref}[ln:formal:PPCMEM Litmus Test]
Lines~\lnref{reginit}-\lnref{P0fail1} are the lines of code
for each process. A given process
can have empty lines, as is the case for P0's line~\lnref{P0empty} and P1's
lines~\lnref{P1empty:b}-\lnref{P1empty:e}.
Labels and branches are permitted, as demonstrated by the branch
on line~\lnref{P0bne} to the label on line~\lnref{P0fail1}.
...
Lines~\lnref{assert:b}-\lnref{assert:e} show the assertion,
which in this case indicates that we
...
P0's lines~\lnref{reginit} and~\lnref{stw} are equivalent to
the C statement \co{x=1} because
line~\lnref{init:0} defines P0's register \co{r2} to be the address of \co{x}. P0's
lines~\lnref{P0lwarx} and~\lnref{P0stwcx} are the mnemonics for
...
lines~\lnref{P0lwsync}-\lnref{P0isync} is equivalent to
the Linux kernel's \co{atomic_add_return(&z, 0)}.
Finally, line~\lnref{P0lwz} is
roughly equivalent to the C statement \co{r3=y}.

P1's lines~\lnref{reginit} and~\lnref{stw} are equivalent to
the C statement \co{y=1}, line~\lnref{P1sync}
is a memory barrier, equivalent to the Linux kernel statement \co{smp_mb()},
and line~\lnref{P1lwz} is equivalent to the C statement \co{r3=x}.
\end{lineref}

\QuickQuiz{}
	\begin{lineref}[ln:formal:PPCMEM Litmus Test]
	But whatever happened to line~\lnref{P0fail1} of
	Listing~\ref{lst:formal:PPCMEM Litmus Test},
...
\QuickQuiz{}
	\begin{lineref}[ln:formal:PPCMEM Litmus Test]
	Is the \co{lwsync} on line~\lnref{P0lwsync} in
	Listing~\ref{lst:formal:PPCMEM Litmus Test} good enough?
	\end{lineref}
\QuickQuizAnswer{
	...
	BoqunFeng2015:powerpc:value-returning-atomics}.
} \QuickQuizEnd
