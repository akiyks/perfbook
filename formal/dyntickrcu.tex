...
interface, the interrupt/\acr{nmi} interface, and the use of
...
functions handle interrupt/\IXacr{nmi} entry and exit, respectively.
...
entry to an interrupt or \acr{nmi} handler (in \co{rcu_irq_enter()})
and is decremented upon exit (in \co{rcu_irq_exit()}).
In addition, the pre-existing \co{in_interrupt()} primitive is
used to distinguish between an outermost or a nested interrupt/\acr{nmi}\@.
...
\QuickQuizB{
	...
	This fails in presence of \IXacrpl{nmi}.
	To see this, suppose an \acr{nmi} was received just after
	\co{rcu_irq_enter()} incremented \co{rcu_update_flag},
	but before it incremented \co{dynticks_progress_counter}.
	The instance of \co{rcu_irq_enter()} invoked by the \acr{nmi}
	...
	\acr{nmi} handler was executing on this CPU, so that any RCU read-side
	critical sections in the \acr{nmi} handler would lose their RCU protection.

	The possibility of \acr{nmi} handlers, which, by definition cannot
...
adding assertions, interrupts, and finally \IXacrpl{nmi}.
...
\begin{quote}
	...
	and didn't take any interrupts, \IXacrpl{nmi}, SMIs, or whatever,
...
states, passing without errors.
However, this version of the model does not handle \IXacrpl{nmi},
which are taken up in the next section.

\subsubsection{Validating NMI Handlers}
\label{sec:formal:Validating NMI Handlers}

We take the same general approach for \IXacrpl{nmi} as we do for interrupts,
keeping in mind that \acrpl{nmi} do not nest.
...
Of course, the fact that we have \acrpl{nmi} requires adjustments in
the other components.
For example, the \co{EXECUTE_MAINLINE()} macro now needs to
pay attention to the \acr{nmi} handler (\co{in_dyntick_nmi}) as well
...
\begin{enumerate}
\item	{\bf Promela and Spin can verify interrupt\slash \IXacr{nmi}\-/handler
	interactions.}
...
due to the fact that both \IXacrpl{irq} and \IXacrpl{nmi} use
the same code path and the
same state variables.
This leads to the notion of providing separate code paths and variables
for \acrpl{irq} and \acrpl{nmi}, as has been done for
...
are for the \IXacr{irq} code paths, and the \co{dynticks_nmi} and
\co{dynticks_nmi_snap} variables are for the \IXacr{nmi} code paths, although
the \acr{nmi} code path will also reference (but not modify) the
...
\item	[\tco{dynticks_nesting}]
	...
	\IXacr{irq} nesting level, otherwise it is one greater than the
	\acr{irq} nesting level.
\item	[\tco{dynticks}]
	This counter's value is even if the corresponding CPU is
	in dynticks-idle mode and there are no \acr{irq} handlers currently
	running on that CPU, otherwise the counter's value is odd.
	In other words, if this counter's value is odd, then the
	corresponding CPU might be in an RCU read-side critical section.
\item	[\tco{dynticks_nmi}]
	This counter's value is odd if the corresponding CPU is
	in an \IXacr{nmi} handler, but only if the \acr{nmi} arrived while this
	CPU was in dyntick-idle mode with no \IXacr{irq} handlers running.
...
\QuickQuiz{
	But what happens if an \IXacr{nmi} handler starts running before
	an \IXacr{irq} handler completes, and if that \acr{nmi} handler continues
	running until a second \acr{irq} handler starts?
}\QuickQuizAnswer{
	This cannot happen within the confines of a single CPU\@.
	The first \acr{irq} handler cannot complete until the \acr{nmi} handler
	returns.
...
\Clnref{irq_sv,irq_rs} disable and reenable \IXacrpl{irq}.
...
which inform RCU of \IXacr{nmi} entry and exit, respectively, from dynticks-idle
mode.
However, if the \acr{nmi} arrives during an \IXacr{irq} handler,
then RCU will already
...
inform RCU of entry to and exit from, respectively, \IXacr{irq} context.
...
RCU read-side critical sections that the subsequent \IXacr{irq} handler
might execute.
...
read-side critical sections that the prior \IXacr{irq} handler might have executed.
...
if the prior \acr{irq} handlers enqueued any
...
neither \IXacrpl{irq} nor \IXacrpl{nmi} in progress (in other words, both snapshots
...
\QuickQuiz{
	...
	the bit when entering an \IXacr{irq} or \IXacr{nmi} handler, and setting it
	upon exit?
}\QuickQuizAnswer{
	Although this approach would be functionally correct, it
	would result in excessive \acr{irq} entry/exit overhead on
	large machines.
	In contrast, the approach laid out in this section allows
	each CPU to touch only per-CPU data on \acr{irq} and \acr{nmi} entry/exit,
	resulting in much lower \acr{irq} entry/exit overhead, especially
	on large machines.
}\QuickQuizEnd
...
sharing between \IXacr{irq} and \IXacr{nmi} contexts.
The only sharing in this simplified interface is references from \acr{nmi}
context to \acr{irq} variables (the \co{dynticks} variable).
This type of sharing is benign, because the \acr{nmi} functions never update
this variable, so that its value remains constant through the lifetime
of the \acr{nmi} handler.
...
where an \IXacr{nmi} might change shared state at any point during execution of
the \IXacr{irq} functions.
