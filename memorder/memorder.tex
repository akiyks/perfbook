...
use of standard \IXpl{mutual-exclusion mechanism} such as locking.
...
\IX{memory ordering}, that, with practice, will be sufficient to implement
...
Unfortunately, the lack of \IXpl{memory barrier} dashes these hopes.
...
CPUs therefore come equipped with \IXpl{store buffer}, as shown in
...
\QuickQuizB{
	...
	\IXalth{lightweight memory-barrier}{read}{memory barrier}
	instructions that order prior loads against
	subsequent loads.
	Similarly,
	the \co{smp_wmb()} row captures the fact that many CPUs have
	\IXalth{lightweight memory-barrier}{write}{memory barrier}
	instructions that order prior stores against
	subsequent stores.
	...
	need not interfere with \IXpl{store buffer}, whose main purpose in life
...
``\IX{single-copy atomic}''~\cite{Stone:1995:SP:623262.623912},
...
\emph{\IX{multicopy atomic}}~\cite{Stone:1995:SP:623262.623912}
...
\emph{\IXalt{other-multicopy atomicity}{other-multicopy atomic}}~\cite[Section B2.3]{ARMv8A:2017},
...
in the meantime, \IXalt{non-multicopy-atomic}{non-multicopy atomic}
platforms do exist, and so software
must deal with them.
...
\QuickQuiz{
	...
	enabled to provide improved \IX{throughput}.
...
even with a \IXh{full}{memory barrier}.
...
two-part guarantee:
(1)~If any part of a given \IXrh{RCU read-side}{critical section} precedes
...
grace-period guarantee, as can be seen by running
\IXaltr{\co{herd}}{herd} on this litmus test.
...
list of questions:\footnote{
	...
	early work on \IXacr{lkmm}, and a few more of which came from other
...
are RCU read-side critical sections, the cycle is forbidden.\footnote{
	Interestingly enough, \ppl{Alan}{Stern} proved that within the context
	of \IXacr{lkmm}, the two-part fundamental property of RCU expressed
...
\cref{sec:memorder:Alpha}.
The short version is that Alpha requires \IXpl{memory barrier} for readers
...
These primitives generate code only in \IXacr{smp} kernels, however, several
...
\begin{description}
\item	[\tco{DMB}] (\IXh{data}{memory barrier}) causes the specified type of
...
instructions.
These instructions act as ``\IXhpl{half}{memory barrier}'', so that
...
Itanium offers a \IXalthalt{weak consistency}{weak}{memory consistency}
...
\QuickQuiz{
	Given that hardware can have a \IXh{half}{memory barrier}, why don't
...
\item[\tco{SYNC_RELEASE}]
	\IXh{Release}{memory barrier}, which may be used to implement
	C++'s \co{atomic_thread_fence(memory_order_release)}.
	In theory, it could also be used to implement the v4.13 Linux-kernel
	\co{smp_store_release()} primitive, but in practice
	\co{sync} is used instead.
\item[\tco{SYNC_RMB}]
	\IXh{Read}{memory barrier}, which could in theory be used to implement the
...
The fourth and final rule of thumb identifies where \IXhpl{full}{memory barrier}
