...
often provide a useful alternative to learning the full \IXacrmf{lkmm}\@.
...
\QuickQuiz{
	...
	Finally, there are now better ways of describing the \acrmf{lkmm}.
...
details of the \IXacrf{lkmm},\footnote{
	Or just ``\acrml{lkmm}'' for brevity.}
the better to get on with
...
Most CPU vendors interested in providing \IX{multicopy atomicity} therefore
...
in ``\IXalth{other-multicopy atomicity}{other-}{multicopy atomicity}''.
...
Within the context of the \IXacrml{lkmm}, the simple answer
is ``yes''.
...
\IXpl{memory-consistency model}, the next sections describe those of a few
...
Its \IXalt{memory model}{memory-consistency model}
is similar to that of \Power{}
...
\ARMv8's \IXalt{memory model}{memory-consistency model}
