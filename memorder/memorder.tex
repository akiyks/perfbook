...
\begin{tabularx}{4.5in}{lrX}
	...
	\multicolumn{3}{r}{\emph{Applies to Linux kernel v5.9 and later.}} \\
\end{tabularx}
...
The ``Relaxed Store'' row also covers the store portion of an atomic RMW operation.
In addition, the ``Relaxed Load'' row covers the load
component of a successful value-returning \co{_relaxed()} RMW atomic
operation, although the combined ``Relaxed RMW operation''
...
``Relaxed RMW operation'' line also applies to unsuccessful
...
the ``Successful full non-\co{void} RMW'' row covers
...
failure, as indicated by the earlier ``Relaxed RMW operation'' row.
...
\QuickQuizB{
	...
	The ``Relaxed Store'' row captures the fact that accesses to
	...
	The ``Relaxed Load'' row captures the fact that (as of 2018) compilers
	...
	The ``Relaxed RMW operation'' row captures the fact
...
%%% smp_read_barrier_depends() was removed as kernel API in Linux v5.9. %%%
...
%	there is little further need for \co{smp_read_barrier_depends()}.
	...
	The \co{rcu_dereference()} row captures the address and data
	dependency ordering provided by \co{rcu_dereference()}.
	...
	The ``Successful full non-\co{void} RMW'' row captures
	...
	is covered by the ``Relaxed RMW operation'' row.
...
\begin{fcvref}[ln:memorder:Insert and Lock-Free Search (No Ordering Before v4.15)]
The dependent-load difference between Alpha and the other CPUs is
illustrated by the code shown in
\cref{lst:memorder:Insert and Lock-Free Search (No Ordering Before v4.15)}.
...
\begin{listing}[tbp]
\begin{fcvlabel}[ln:memorder:Insert and Lock-Free Search (No Ordering Before v4.15)]
\begin{VerbatimL}[commandchars=\\\[\]]
...
\end{VerbatimL}
\end{fcvlabel}
\caption{Insert and Lock-Free Search (No Ordering Before v4.15)}
\label{lst:memorder:Insert and Lock-Free Search (No Ordering Before v4.15)}
\end{listing}
...
\Cref{fig:memorder:Why smp-read-barrier-depends() was Required (Before v4.15)}
...
\caption{Why \tco{smp_read_barrier_depends()} was Required (Before v4.15)}
\label{fig:memorder:Why smp-read-barrier-depends() was Required (Before v4.15)}
\end{figure}
...
A \co{smp_read_barrier_depends()} primitive had therefore been added to the
...
\cref{lst:memorder:Safe Insert and Lock-Free Search}.\footnote{
	\co{smp_read_barrier_depends()} is no longer available
	in Linux kernel versions v5.9 and later.}
\end{fcvref}
...
Alpha is the only CPU where \co{smp_read_barrier_depends()} was
an \co{smp_mb()} rather than a no-op.

\QuickQuizSeries{%
\QuickQuizB{
	Why was Alpha's \co{smp_read_barrier_depends()} an
	...
	in the core kernel.\footnote{
		\co{smp_read_barrier_depends()} is no longer available
		in the Linux kernel since v5.9.
		Alpha-specific code now uses its own memory-ordering
		primitives instead.}
	This means that other than the Alpha architecture-specific code,
	no Linux-kernel
