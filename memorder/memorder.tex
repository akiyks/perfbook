...
\begin{description}
\item	[\apitk{smp_mb()}] (\IXh{full}{memory barrier}) that orders both loads and
	stores.
...
\item	[\apitk{smp_rmb()}] (\IXh{read}{memory barrier}) that orders only loads.
\item	[\apitk{smp_wmb()}] (\IXh{write}{memory barrier}) that orders only stores.
\item	[\apitk{smp_mb__before_atomic()}] that forces ordering of accesses
...
\item	[\apitk{smp_mb__after_atomic()}] that forces ordering of accesses
...
\item	[\apitk{smp_mb__after_spinlock()}] that forces ordering of accesses
...
\item	[\apitk{mmiowb()}] that forces ordering on MMIO writes that are guarded
