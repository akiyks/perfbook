...
%
\Epigraph{The art of progress is to preserve order amid change and to preserve change amid order.}{\pplEPmdl{Alfred North}{Whitehead}}
...
either \apik{smp_mb__before_atomic()} or \apik{smp_mb__after_atomic()}.
...
\epigraph{Nothing is orderly till people take hold of it.
	 ...
	 {\pplEPmdl{Henry Ward}{Beecher}, updated}
...
the illusion of ordering through use of \emph{\IXB{memory barrier}s} (for
...
\epigraph{Knowing where the trap is---that's the first step in evading it.}
	 {Duke Leto Atreides, \emph{Dune}, \pplEP{Frank}{Herbert}}
...
\IX{Plain access}es, as in plain-access C-language assignment statements such
...
avoided by avoiding \IX{data race}s on plain accesses, as described in
...
and \IXhplx{data}{dependenc}{ies}{y} discussed in
\cref{sec:memorder:Address Dependencies,%
...
The \IXhplx{control}{dependenc}{ies}{y} described in
...
\epigraph{Method will teach you to win time.}
	 {\pplEPmdl{Johann Wolfgang von}{G\"othe}}
...
Instead, the \apik{smp_mb__after_spinlock()} and
\apik{smp_mb__after_unlock_lock()} primitives are provided
for those more complex use cases, as discussed
...
%
\epigraph{Rock beats paper!}{\pplEP{Derek}{Williams}}
...
\begin{description}
\item	[\apiDtk{smp_mb()}] (\IXh{full}{memory barrier}) that orders both loads and
	stores.
...
\item	[\apiDtk{smp_rmb()}] (\IXh{read}{memory barrier}) that orders only loads.
\item	[\apiDtk{smp_wmb()}] (\IXh{write}{memory barrier}) that orders only stores.
\item	[\apiDtk{smp_mb__before_atomic()}] that forces ordering of accesses
...
\item	[\apiDtk{smp_mb__after_atomic()}] that forces ordering of accesses
...
\item	[\apiDtk{smp_mb__after_spinlock()}] that forces ordering of accesses
...
\item	[\apiDtk{mmiowb()}] that forces ordering on MMIO writes that are guarded
...
alternating \IX{cache line}s are processed by the different partitions
of the caches.
...
\IXhplx{address}{dependenc}{ies}{y} on the read side.
...
\ARM\ also implements \IXhplx{control}{dependenc}{ies}{y},
