...
\begin{tabular}{lrl}
	Key:	& C: & Ordering is cumulative \\
		...
		& a: & Provides specified ordering given intervening atomic RMW operation \\
...
\QuickQuizB{
	...
	\IXalt{atomic RMW operations}{atomic read-modify-write operation}
	provide full ordering in cases where these
	operations succeed.
...
CPUs agree on the order of all stores.\footnote{
	As of early 2021, \ARMv8 and x86 provide
        \IXalt{other-multicopy atomicity}{other-multicopy atomic},
	IBM mainframe provides full \IXalt{multicopy atomicity}{multicopy atomic}, and PPC
...
Unlike multicopy-atomic platforms, within
\IXalt{other-multicopy-atomic}{other-multicopy atomic} platforms,
...
\QuickQuiz{
	...
	\IX{multicopy atomic} on the one hand and \IX{other-multicopy atomic}
	on the other?
}\QuickQuizAnswer{
\begin{listing}[tbp]
...
demonstrates \IXalt{multicopy atomicity}{multicopy atomic},
that is, on a multicopy-atomic platform,
the \co{exists} clause on \clnref{exists} cannot trigger.
In contrast, on a \IXalt{non-multicopy-atomic}{non-multicopy atomic}
...
\IXalt{non-multicopy-atomic}{non-multicopy atomic} behaviors in some situations.
...
\IXalt{Plain accesses}{plain access},
...
memory.
Full ordering must therefore be provided for
\IXalt{plain accesses}{plain access}, in spite of
...
The next two rows cover \IXalt{multicopy atomicity}{multicopy atomic},
...
weaker \IXalt{other-multicopy atomicity}{other-multicopy atomic}.
...
\item	[\tco{smp_mb__before_atomic()}] that forces ordering of accesses
	...
	a later \IXalt{atomic RMW operation}{atomic read-modify-write operation}.
...
This software barrier could be implemented by sending \IXacrpl{ipi}
to all other CPUs.
Upon receipt of such an \acr{ipi}, a CPU would execute a memory-barrier
...
\QuickQuizB{
	...
	and a few of Alpha's \IXpl{atomic read-modify-write operation},
...
\IXalt{other-multicopy atomicity}{other-multicopy atomic}, for example,
...
different CPUs, that is, the z~Systems CPU family is fully \IX{multicopy
...
\IXalt{atomic RMW operations}{atomic read-modify-write operation},
and other atomic RMW operations in conjunction with
