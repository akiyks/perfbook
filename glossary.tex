...
\begin{description}
\item[Associativity:]\index{Cache geometry!associativity}
  \index{Associativity|see{Cache geometry, associativity}}
...
\item[Associativity Miss:]\index{Cache miss!associativity}
...
\item[Cache Line:]\index{Cache geometry!line}
  \index{Cache line|see{Cache geometry, line}}
...
\item[Communication Miss:]\index{Cache miss!communication}
...
\item[Data Locking:]\index{Locking!data}
...
\item[Direct-Mapped Cache:]\index{Cache!direct-mapped}
...
\item[Embarrassingly Parallel:]\index{Embarrassingly parellel}
	...
	CPUs are available).
\item[Exclusive Lock:]\index{Lock!exclusive}
...
\item[Fragmentation:]\index{Fragmentation}
	...
	actually requested.
\item[Fully Associative Cache:]\index{Cache!fully associative}
...
\item[Grace Period:]\index{RCU!grace period}
  \index{Grace period|see{RCU, grace period}}
...
\item[MESI Protocol:]\index{Cache-coherence protocol!MESI}
  \index{MESI protocol|see{Cache-coherence protocol, MESI}}
	The
...
\item[Process Consistency:]\index{Memory consistency!process}
...
\item[Quiescent State:]\index{RCU!quiescent state}
  \index{Quiescent state|see{RCU, quiescent state}}
...
\item[Read-Copy Update (RCU):]\index{RCU}
  \index{Read-copy update|see{RCU}}
...
\item[Read-Side Critical Section:]\index{Critical section!read-side}
...
\item[Reader-Writer Lock:]\index{Lock!reader-writer}
...
\item[Sequential Consistency:]\index{Memory consistency!sequential}
...
\item[Transactional Memory (TM):]\index{Transactional memory}
  \index{TM|see{Transactional memory}}
...
\item[Write Miss:]\index{Cache miss!write}
...
\item[Write-Side Critical Section:]\index{Critical section!write-side}
