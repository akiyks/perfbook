...
For example, if CPU~0 were to perform a
\IXalt{compare-and-swap (CAS)}{CAS} operation on a
...
\IXalt{atomic operations}{Atomic operation}
on the lock data structure, one for acquisition and
...
A final hardware optimization is
\IXalt{read-mostly replication}{Read-mostly replication}, in which
