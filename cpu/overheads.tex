...
For example, if CPU~0 were to perform a \IXacr{cas} operation on a
...
\item	CPU~0's and 1's interconnect forwards the cacheline to CPU~0's
	cache.
\item	CPU~0 can now perform the \IXacr{cas} operation on the value in its cache.
\end{enumerate}
...
\QuickQuizB{
	...
	\item	Other CPUs might be concurrently attempting to perform
		\IXacr{cas} operations involving this same cacheline.
...
The same-CPU \IXacr{cas} operation consumes about seven
nanoseconds, a duration more than ten times that of the clock period.
Here, ``same-CPU'' means that the same CPU now performing the \acr{cas} operation
...
The lock operation is more expensive than \IXacr{cas} because it requires two
\IXpl{atomic operation} on the lock data structure, one for acquisition and
...
\IXAcr{cas} stands for an atomic compare-and-swap operation, where the hardware
...
in which case the \acr{cas} operation is said to have succeeded.
If they compare unequal, the memory location keeps its (unexpected) value,
and the \acr{cas} operation is said to have failed.
...
\Acr{cas} functionality is provided by the \co{lock;cmpxchg} instruction on x86.
...
is represented by the value one, then a \acr{cas} operation on the lock
...
location, namely the \acr{cas} operation itself.

In contrast, a normal \acr{cas} operation's old value is derived from
some earlier load.
...
new value.
Then in the \acr{cas} operation, the value actually loaded would be specified
...
not match, causing a miscompare that would result in the \acr{cas} operation
failing.
The key point is that there are now two accesses to the memory location,
the load and the \acr{cas}.

Thus, it is not surprising that in-core blind \IXacr{cas} consumes only about
...
An blind \IXacr{cas} involving CPUs in different cores but on the same socket
...
A non-blind \acr{cas} operation, which as noted earlier must look at the old
...
In the time required to do \emph{one} \acr{cas} operation, the CPU could have
...
more expensive.
A blind \IXacr{cas} operation consumes almost 150~nanoseconds, or more than
three hundred clock cycles.
A \acr{cas} operation consumes more than 400~nanoseconds, or almost
...
In this case, a blind \IXacr{cas} operation consumes more than three hundred
nanoseconds, or more than seven hundred clock cycles.
A \acr{cas} operation consumes almost a full microsecond, or almost two
