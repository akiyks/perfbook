...
\emph{\IXalt{cache-coherency protocols}{cache-coherence protocol}}~\cite{
Hennessy95a,DavidECuller1999,MiloMKMartin2012scale,DanielJSorin2011MemModel},
which is discussed in more detail in
Appendix~\ref{chp:app:whymb:Why Memory Barriers?}.
As can be seen in the sequence of events triggered by a \IXacrm{cas} operation,
...
The same-CPU \IXacrmf{cas} operation consumes about seven
nanoseconds, a duration more than ten times that of the clock period.
\Acr{cas} is an atomic operation in which the hardware compares the contents
...
\Acrm{cas} functionality is provided by the \co{lock;cmpxchg} instruction on x86.

The ``same-CPU'' prefix means that the CPU now performing the \acrm{cas} operation
...
The lock operation is more expensive than \acr{cas} because it requires two
...
In the case of the blind \IXacr{cas}, the software specifies the old value
...
is represented by the value one, then a \acrm{cas} operation on the lock
...
location, namely the \acrm{cas} operation itself.

In contrast, a normal \acrm{cas} operation's old value is derived from
some earlier load.
...
new value.
Then in the \acrm{cas} operation, the value actually loaded would be specified
as the old value and the incremented value as the new value.
If the value had not been changed between the load and the \acr{cas}, this
would increment the memory location.
However, if the value had in fact changed, then the old value would
not match, causing a miscompare that would result in the \acrm{cas} operation
failing.
The key point is that there are now two accesses to the memory location,
the load and the \acr{cas}\@.

Thus, it is not surprising that in-core blind \acr{cas} consumes only about
seven nanoseconds, while in-core \acr{cas} consumes about 18 nanoseconds.
...
A blind \IXacr{cas} involving CPUs in different cores but on the same socket
...
back and forth between a pair of CPUs, so this \IX{cache miss} is satisfied
not from memory, but rather from the other CPU's cache.
A non-blind \acrm{cas} operation, which as noted earlier must look at the old
...
In the time required to do \emph{one} \acrm{cas} operation, the CPU could have
...
more expensive.
A blind \IXacrm{cas} operation consumes almost 150~nanoseconds, or more than
three hundred clock cycles.
A normal \acrm{cas} operation consumes more than 400~nanoseconds, or almost
...
In this case, a blind \acrm{cas} operation consumes more than three hundred
nanoseconds, or more than seven hundred clock cycles.
A \acrm{cas} operation consumes almost a full microsecond, or almost two
