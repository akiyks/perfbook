...
\IXalth{}{Pipelined CPU|(}{CPU!pipelined|(}%
...
instructions simultaneously, using \emph{pipelines};
\emph{\IXalth{superscalar}{Superscalar CPU}{CPU!superscalar}} techniques;
\emph{\IXalt{out-of-order}{Out-of-order execution}}
instruction and data handling;
\emph{\IXalt{speculative execution}{Speculative execution}}, and
more~\cite{Hennessy2011}
in order to optimize the flow of instructions and data through the CPU.
Some cores have more than one \IXalt{hardware thread}{Hardware thread},
which is variously called
\emph{\IXalt{simultaneous multithreading}{Simultaneous multithreading (SMT)}}
(SMT) or \emph{\IXalt{hyperthreading}{Hyperthreading (HT)}}
...
This gets even worse in the increasingly common case of
\IXalt{hyperthreading}{Hyperthreading (HT)}
(or \IXalt{SMT}{Simultaneous multithreading (SMT)},
if you prefer), especially on
\IXalth{pipelined}{Pipelined CPU}{CPU!pipelined}
\IXalth{superscalar}{Superscalar CPU}{CPU!superscalar}
\IXalt{out-of-order}{Out-of-order execution}
CPU featuring \IXalt{speculative execution}{Speculative execution}.
In this increasingly common case, all the
\IXalt{hardware threads}{Hardware thread} sharing
a core also share that core's resources, including registers, cache,
execution units, and so on.
The instructions are often decoded into
\IXalt{micro-operations}{Micro-operation}, and use of the
