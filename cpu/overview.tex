...
\IXalthr{}{Pipelined CPU|(}{CPU}{pipelined|(}%
...
\emph{\IXalthr{superscalar}{Superscalar CPU}{CPU}{superscalar}} techniques;
\emph{\IXalt{out-of-order}{out-of-order execution}}
instruction and data handling;
\emph{\IX{speculative execution}}, and
more~\cite{Hennessy2011}
in order to optimize the flow of instructions and data through the CPU.
Some cores have more than one \IX{hardware thread}, which is variously called
\IXacr{smt} or \IXacr{ht}~\cite{JFennel1973SMT},
...
then proceed using \IX{speculative execution}.
...
This gets even worse in the increasingly common case of \IXacr{ht}
(or \IXacr{smt}, if you prefer), especially on pipelined
\IXalthr{superscalar}{Superscalar CPU}{CPU}{superscalar}
\IXalt{out-of-order}{out-of-order execution}
CPU featuring \IX{speculative execution}.
In this increasingly common case, all the \IXpl{hardware thread} sharing
a core also share that core's resources, including registers, cache,
execution units, and so on.
The instructions are often decoded into \IXpl{micro-operation}, and use of the
...
Even if only one \IX{hardware thread} is active (for example, in old-school
