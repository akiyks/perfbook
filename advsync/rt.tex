...
long enough to make the \acr{irq} thread aware of the new event.
...
the context of the \acr{irq} thread may be preempted by high-priority real-time
threads.
...
is deferred until the \acr{irq} thread gets around to running it.
...
If an interrupt occurs, a high-priority \acr{irq} thread will preempt one
...
doing.
So the high-priority \acr{irq} thread cannot acquire the lock until after one
of the medium-priority threads releases its CPU\@.
In short, the medium-priority threads are indirectly blocking the
high-priority \IXacr{irq} threads, a classic case of priority inversion.
