...
that response time is more important than \IX{throughput} in
real-time systems.
...
within \IXrhpl{RCU read-side}{critical section},
...
provides acceptably fine timer granularity for most (but not
all!) workloads, but which processes timers so frequently that the system
...
A \IX{grace period} is permitted to end: (1)~Once all CPUs have completed any
...
\ppl{Frederic}{Weisbecker}~\cite{JonCorbet2013NO-HZ-FULL,FredericWeisbecker2013nohz}
...
For parallel real-time applications, \IXhpl{synchronization}{overhead} clearly
...
\IXhpl{sequence}{lock} and \IXpl{hazard pointer}.
