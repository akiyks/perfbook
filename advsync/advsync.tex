...
gives a brief overview of \IXacrl{nbs}.
...
The term \IXacr{nbs}~\cite{MauriceHerlihy90a}
...
\Acr{nbs} classes~1, 2, and~3 were first formulated in the early 1990s,
...
but there are a relatively small subset of \acr{nbs} algorithms that are
in common use.
...
Perhaps the simplest \IXacr{nbs} algorithm is atomic update of an integer
counter using fetch-and-add (\co{atomic_add_return()}) primitives.

\subsubsection{NBS Sets}
\label{sec:advsync:NBS Sets}

Another simple \acr{nbs} algorithm implements a set of integers in an array.
...
a set member.
The linearizability criterion for \acr{nbs} algorithms requires that reads from
...
An \IXacr{nbs} set may also be implemented using a bitmap, where each value that
...
instructions, although \IXacrl{cas} (\co{cmpxchg()} or \acr{cas})
...
This algorithm is probably the most heavily used \IXacr{nbs} algorithm in the
Linux kernel.
...
Another common \IXacr{nbs} algorithm is the atomic queue where elements are
...
This results in a half-\IXacr{nbs} algorithm where enqueues are \acr{nbs} but
...
bounded wait-free pop (\path{lifo-push.c}), forming an \acr{nbs} stack.
...
The most heavily cited \IXacr{nbs} benefits stem from its forward-progress
...
\IXacr{nbs}'s forward-progress guarantees have caused many to suggest its use in
real-time systems, and \acr{nbs} algorithms are in fact used in a great many
such systems.
...
\item	Real-time forward-progress guarantees usually have some
	...
	In contrast, the most popular forms of \acr{nbs} only guarantees
...
\item	Real-time forward-progress guarantees are often
	...
	In contrast, many of \acr{nbs}'s forward-progress guarantees are
	unconditional.
\item	Real-time forward-progress guarantees are often conditioned on
	...
	In contrast, \acr{nbs}'s forward-progress
	guarantees are often unconditional, although recent \acr{nbs} work
...
\item	An important component of a real-time program's environment
	is the scheduler.
	\IXacr{nbs} algorithms assume a worst-case \emph{demonic scheduler}.
	...
	\acr{nbs}~\cite{DanAlitarh2013PracticalProgress,BjoernBrandenburgPhD}.
\item	Real-time forward-progress guarantees usually apply only
	in the absence of software bugs.
	In contrast, many classes of \acr{nbs} guarantees apply even in the
	face of fail-stop bugs.
\item	\acr{nbs} forward-progress guarantee classes imply linearizability.
	In contrast, real-time forward progress guarantees are often
	independent of ordering constraints such as linearizability.
\end{enumerate}

To reiterate, despite these differences, a number of \IXacr{nbs} algorithms are
extremely useful in real-time programs.

\subsubsection{NBS Fail-Stop Tolerance}
\label{sec:advsync:NBS Fail-Stop Tolerance}

Of the classes of \IXacr{nbs} algorithms, wait-free synchronization (bounded or
...
Nevertheless, there are a few applications where \IXacr{nbs}'s rather limited
...
Nevertheless, \acr{nbs} is not a panacea even within this restricted area,
...
automatically covers simple \IXacr{nbs} algorithms.

However, the linearization points of a complex \acr{nbs} algorithms are often
...
of complex \acr{nbs} algorithms should be regarded with deep
...
however, such queues are much more complex than the half-\IXacr{nbs} algorithm
outlined above.
...
algorithms that provide only \IXacrl{nbs},
both in theory~\cite{DanAlitarh2013PracticalProgress}
