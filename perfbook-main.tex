...
\newboolean{msnt}
\setboolean{msnt}{false}
\newboolean{mstx}
\setboolean{mstx}{false}
\newboolean{mss}
\setboolean{mss}{false}
\newboolean{msr}
\setboolean{msr}{false}
\newboolean{msn}
\setboolean{msn}{false}
\newboolean{msns}
\setboolean{msns}{false}
\newboolean{msdefault}
\setboolean{msdefault}{true}

\newboolean{newtxoneseven}
\setboolean{newtxoneseven}{false} % newtx version 1.7 or later (set by Makefile)
\input{fontconfig.sty} % Load perfbook specific font config

% mitigate harmless warnings from tcolorbox >=v6.0.0 (for lualatex)
\ifluatex
  \usepackage{luacolor}
\fi
...
\usepackage{siunitx} % for \num{}, \unit{}, and \qty{} macros and "S" column in tabular
}{
\usepackage{siunitx}[=v2] % siunitx >= 3.0 needs unicode math fonts
}
