...
\QuickQuiz{
	If the C language makes no guarantees in presence of a \IX{data	race},
	then why does the Linux kernel have so many data races?
...
The POSIX standard allows the programmer to avoid \IXpl{data race} via
``POSIX locking''.
...
This \IXalth{exclusive-locking}{exclusive}{lock}
property is demonstrated using the code shown in
...
Although there is quite a bit more to POSIX
\IXalth{exclusive locking}{exclusive}{lock}, these
...
The POSIX API provides a \IXh{reader-writer}{lock}, which is represented by
...
(\path{rwlockscale.c})
shows one way of measuring \IXh{reader-writer}{lock} scalability.
...
\IXalth{reader-writer locking}{reader-writer}{lock}
scalability is
...
\QuickQuizM{
	...
	\IXh{reader-writer}{lock} by acquiring its own lock, and write-acquires
...
\QuickQuizM{
	...
	be faster.
	So why should anyone worry about \IXhpl{reader-writer}{lock} being slow?
}\QuickQuizAnswerE{
...
The classic \IXacr{cas} operation is provided by a pair of
...
Either of the \IXacrl{cas} operation is ``universal'' in the sense
that any atomic operation on a single location can be implemented in
terms of \acrl{cas}, though the earlier operations are often
more efficient where they apply.
The \acrl{cas} operation is also capable of serving as the basis
...
\apic{atomic_signal_fence()}), and atomic read-modify-write operations.
The \IXpl{atomic read-modify-write operation} include
...
\QuickQuiz{
	...
	This is because the compiler is forbidden from introducing
	\IXpl{data race}.
...
against compiler optimizations that invent \IXpl{data race}.
...
Ordering is also provided by some \IXpl{atomic read-modify-write operation},
some of which are presented in
Section~\ref{sec:toolsoftrade:Atomic Operations}.
...
\subsubsection{Avoiding \IXpl{Data Race}}
...
allowing \IXalt{plain loads and stores}{plain access}
for some accesses to a given variable,
while requiring \IXalt{markings}{marked access}
(such as \apik{READ_ONCE()} and \apik{WRITE_ONCE()})
...
\QuickQuiz{
	...
	may safely use \IXalt{plain accesses}{plain access},
	and even then only if those
...
the celebrated \IXacrf{cas} operation is provided by
\apik{atomic_cmpxchg()}.
