...
\QuickQuizM{
	...
	Don't we also need \IXpl{memory barrier} to make sure
...
\begin{equation}
	\frac{L_N}{N L_1}
\end{equation}
\noindent where $N$ is the number of threads,
...
\QuickQuizSeries{%
\QuickQuizB{
	Isn't comparing against single-CPU \IX{throughput} a bit harsh?
}\QuickQuizAnswerB{
	Not at all.
...
to create their own implementations for situations where the
\IXh{full}{memory ordering} semantics are not required.
...
For example, a \IXh{write}{memory barrier} (Linux kernel \apik{smp_wmb()}) would
...
instruction for a \IXh{full}{memory barrier}, for example, \co{"mfence"}
...
specified (these are very handy for managing \IXpl{reference counter}, for
example).
