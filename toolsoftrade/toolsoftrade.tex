...
Processes are created using the \api{fork()}{fork} primitive, they may
be destroyed using the \api{kill()}{kill} primitive, they may destroy
themselves using the \api{exit()}{exit} primitive.
A process executing a \co{fork()} primitive is said to be the ``parent''
of the newly created process.
A parent may wait on its children using the \api{wait()}{wait} primitive.
...
to the \api{waitall()}{waitall} function shown in
...
\api{pthread_create()}{pthread create} primitive, for example, as shown on
...
call \api{pthread_exit()}{pthread exit}.
...
\begin{fcvref}[ln:toolsoftrade:pcreate:mythread]
The \api{pthread_join()}{pthread join} primitive, shown on line~\lnref{join},
is analogous to
