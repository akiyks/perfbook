...
\epigraph{The supreme excellence is simplicity.}
	 {\pplEPmdl{Henry Wadsworth}{Longfellow}, simplified}
% The original:
...
\QuickQuizE{
	...
	For more information on \apik{READ_ONCE()} and \apik{WRITE_ONCE()},
	please see
	...
	intrinsic \apialtg{__atomic_store_n(&x, v, __ATOMIC_RELAXED)}
	{__atomic_store_n()}.
}\QuickQuizEndE
}
...
As you might expect, \IXh{reader-writer}{lock}s are designed for read-mostly
situations.
...
decidedly non-ideal, especially for smaller sizes of \IX{critical
section}s.
...
case the \apiDk{barrier()} primitive may be used.
\begin{fcvref}[ln:toolsoftrade:lock:reader_writer:reader]
...
\apiDk{READ_ONCE()} primitive may be used, as it was on \clnref{read_x} of
\cref{lst:toolsoftrade:Demonstration of Exclusive Locks}.
\end{fcvref}
Similarly, the \apiDk{WRITE_ONCE()} primitive may be used to prevent the
...
code, the code samples in this book start with a call to \apiDpf{smp_init()},
...
\apiDk{struct task_struct} pointers to track kthreads,
\apiDk{kthread_create()} to create them,
\apiDk{kthread_should_stop()} to externally suggest that they stop
(which has no POSIX equivalent),\footnote{
	POSIX environments can work around the lack of
	\co{kthread_should_stop()} by using a properly synchronized
	boolean flag in conjunction with \co{pthread_join()}.}
\apiDk{kthread_stop()} to wait for them to stop, and
\apiDk{schedule_timeout_interruptible()} for a timed wait.
...
Each such thread has an identifier of type \apiDpf{thread_id_t},
...
\item[\apiDtpf{for_each_running_thread()}]
...
Use these primitives to avoid \IX{data race}s, that is, ensure that if
...
\co{p} is usually zero.\footnote{
	The \apiDk{unlikely()} function provides this hint to the compiler,
...
those defined on type \apiDk{atomic_t} provide a good start.
Normal non-tearing reads and stores are provided by
\apiDk{atomic_read()} and \apiDk{atomic_set()}, respectively.
\IX{Acquire load} is provided by \apiDk{smp_load_acquire()} and
\IX{release store} by \apiDk{smp_store_release()}.

Non-value-returning fetch-and-add operations are provided by
\apiDk{atomic_add()}, \apiDk{atomic_sub()}, \apiDk{atomic_inc()}, and
\apiDk{atomic_dec()}, among others.
An atomic decrement that returns a reached-zero indication is provided
by both \apiDk{atomic_dec_and_test()} and \apiDk{atomic_sub_and_test()}.
An atomic add that returns the new value is provided by
\apiDk{atomic_add_return()}.
Both \apiDk{atomic_add_unless()} and \apiDk{atomic_inc_not_zero()} provide
...
An atomic exchange operation is provided by \apiDk{atomic_xchg()}, and
the celebrated \acrmf{cas} operation is provided by
\apiDk{atomic_cmpxchg()}.
...
The Linux kernel uses \apiDk{DEFINE_PER_CPU()} to define a per-CPU variable,
\apiDk{this_cpu_ptr()} to form a reference to this CPU's instance of a
given per-CPU variable, \apiDk{per_cpu()} to access a specified CPU's
...
\begin{description}[style=nextline]
\item[\apiDtpf{DEFINE_PER_THREAD()}]
...
\item[\apiDtpf{DECLARE_PER_THREAD()}]
...
If you need to access a given shared variable both with \IX{plain access}es
