...
\Epigraph{Locking is the worst general-purpose synchronization mechanism
	  ...
	  to the memory of \pplEP{Winston}{Churchill} and to whoever he was
	  quoting}
...
\IX{unfairness}, \IX{data race}s, and all manner of other concurrency sins.
...
%
\epigraph{I work to stay alive.}{\pplEP{Bette}{Davis}}
...
Locks are implemented using atomic instructions and \IX{memory barrier}s,
...
are an effective performance booster, at least for large \IX{critical section}s.
...
\IXh{Exclusive}{lock}s are what they say they are:
...
\IXh{Reader-writer}{lock}s~\cite{Courtois71}
...
\IXh{read-side}{critical section}s, preferably measured in hundreds of microseconds
...
by \pplsur{John M.}{Mellor-Crummey} and
\pplsur{Michael L.}{Scott}~\cite{MellorCrummey91a}.
...
\pplsur{David}{Dice} et al.\ discuss use of local and global locks
...
low contention.
\ppl{Beng-Hong}{Lim} and \ppl{Anant}{Agarwal} therefore combined
...
of contention.
\pplsur{Luke}{Browning} et al.\ took a similar approach,
but avoided the use of a separate
...
based on simple loads and stores may be found in \pplsur{Maurice P.}{Herlihy}'s
and \pplsur{Nir}{Shavit}'s
...
\pplsur{Ben}{Gamsa} et al.~\cite[Section 5.3]{Gamsa99} describe
a token-based mechanism in which a token circulates among the CPUs.
...
special-purpose locking primitives to appear, see for example
\pplsur{Rachid}{Guerraoui} et
...
\epigraph{You either die a hero or you live long enough to see yourself
	  become the villain.}
	 {\pplEP{Aaron}{Eckhart} \emph{as} \pplEP{Harvey}{Dent}}
...
provides \IX{forward-progress guarantee}s, which few do.
