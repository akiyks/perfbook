...
Locks are implemented using \IX{atomic} instructions and memory barriers,
...
It is important to note that unconditionally acquiring an
\IXh{exclusive}{lock} has two effects:
...
\IXalth{exclusive locking}{exclusive}{lock}
for short critical sections: The overhead of acquiring
...
favorable to readers through use of per-thread
\IXhpl{exclusive}{lock}~\cite{WilsonCHsieh92a}.
...
As with exclusive locks, a \IXh{reader-writer}{lock} acquisition cannot complete
...
on \IX{atomic} instructions, many of which are reviewed in the classic paper
...
\item	If the data structures are too complex for reasonable use of
	\acrl{nbs}, create a queue that allows
