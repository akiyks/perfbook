...
\item[OpenMP:] This set of compiler directives can be used
	...
	It is, however, much easier to use than \acr{mpi} or C/C++
...
user, each of which generates \IXacr{sql} from user queries.
This per-user \acr{sql} is run against a common relational database, which
...
hardware such as vector units and \IXacrpl{gpgpu}\@.
...
\IXacrfst{sql}~\cite{DIS9075SQL92}, and
partitioned global address-space (PGAS) environments
such as \IXacr{upc}~\cite{ElGhazawi2003UPC,UPCConsortium2013}
offer implicit access,
while \IXacr{mpi}~\cite{MPIForum2008} offers
...
The poster child for these environments is \IXacr{sql}, many implementations
